...
"""
Re-arranged C++ kernel wrapper to better overlap cudaMemcpyAsync with host side tasks
Pipeline TMA between tiles
"""
...
// into tmem_ptr, repeated down to the next 3 chunks of 32 rows
template<int CTA_GROUP>
...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int TD_MMA_M, int TD_MMA_N, int TD_MMA_K, bool SWIZZLE, int PIPE_STAGES, int NUM_WARPS, int OUT_N_CHUNK, bool SINGLE_TILE>
__global__ void __cluster_dims__(1, 1, 1) nvfp4_group_gemm_kernel(GroupDesc* groups, const __grid_constant__ CUtensorMap tmap_a_temp, 
    ...
    */
    ...
    __shared__ CUtensorMap local_C_tmap;
    ...
    */
    ...
    __shared__ int group_smem, m_off_smem, n_off_smem;
    __shared__ int group_smem_epi, m_off_smem_epi, n_off_smem_epi;
    ...
    const int mbar_addr_mma_done = mbar_addr_mma + PIPE_STAGES * 8;
    const int mbar_addr_clc = mbar_addr_mma_done + 8;
    const int mbar_addr_tile_ready = mbar_addr_clc + 8;
    const int mbar_addr_epi_done = mbar_addr_tile_ready + 8;

    int tile_phase = 0, epi_done_phase = 0, mma_done_phase = 0;

    if (warp_id == 0 && elect_one_sync()) {
        for (int i = 0; i < PIPE_STAGES * 2 + 4; i++) {
            int count = (mbar_addr_tma + i * 8 == mbar_addr_mma_done) ? 2 : 1;
            mbar_init(mbar_addr_tma + i * 8, count);
        }
    ...
    // All warps compute first work tile, so we don't have to synchronize with admin warp on first tile
    ...
    int m_off = (group_tile / n_tiles) * TD_CTA_M;
    int n_off = (group_tile % n_tiles) * TD_CTA_N;
    int last_group = -1;
    ...
    while (true) {
        ...
        if (warp_id == TMA_WARP) {
            ...
            // Exit loop if no more work to be done, tell other warps as much
            if (tile_idx < 0) {
                if (elect_one_sync()) {
                    group_smem = -1;
                    asm volatile("fence.proxy.async::generic.release.sync_restrict::shared::cta.cluster;"); // ISSUE: is this necessary?
                    mbar_arrive_expect(mbar_addr_tile_ready, 0);
                }
                break;
            }

            last_group = group;
            // Compute group for new tile stolen by CLC
            lo = 0; hi = G - 1;
            while (lo <= hi) {
                group = lo + ((hi - lo) / 2);
                if (tile_idx < groups[group].block_start) {
            ...
            group_tile = tile_idx - groups[group].block_start;
            m_off = (group_tile / n_tiles) * TD_CTA_M;
            n_off = (group_tile % n_tiles) * TD_CTA_N;

            // Write new group, m_off, n_off to SMEM
            if (elect_one_sync()) {
                group_smem = group;
                m_off_smem = m_off;
                n_off_smem = n_off;
        ...
        // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops
            ...
            if (elect_one_sync()) {
                ...
                group_smem_epi = group;
                m_off_smem_epi = m_off;
                n_off_smem_epi = n_off;

                // Signal epilogue to start
                mbar_arrive_expect(mbar_addr_mma_done, 0);
                tcgen05_commit(mbar_addr_mma_done);
            }

            /* DEBUG
            if (blockIdx.x == 0 && elect_one_sync()) {
                printf("Finished MMA for group: %d, m_off: %d, n_off: %d\\n", group, m_off, n_off);
            }
            */

            // Wait for TMA warp to publish next tile
            mbar_wait(mbar_addr_tile_ready, tile_phase);
            tile_phase ^= 1;
            ...
            // Wait for epilogue to be done using TMEM
        ...
        // All warps aside from the two for TMA/MMA are dedicated to the epilogue
            ...
            mbar_wait(mbar_addr_mma_done, mma_done_phase);
            mma_done_phase ^= 1;

            group = group_smem_epi;
            m_off = m_off_smem_epi;
            n_off = n_off_smem_epi;
            ...
            if (blockIdx.x == 0 && threadIdx.x == 0) {
                printf("Storing group: %d, m_off: %d, n_off: %d\\n", group, m_off, n_off);
            }
            */
            ...
            if (warp_id == 0 && elect_one_sync()) {
                ...
                mbar_arrive_expect(mbar_addr_epi_done, 0);
            }
        }
    }
    // Free memory
...
struct TimingStats {
    ...
    void record(float memcpy_ms, float kernel_ms) {
        memcpy_us_total += memcpy_ms * 1000.0;
        kernel_us_total += kernel_ms * 1000.0;
        total_us_total += (memcpy_ms + kernel_ms) * 1000.0;
        count++;
    }

    void print_current(float memcpy_ms, float kernel_ms) {
        printf("[nvfp4_group_gemm] cudaMemcpy: %.2f us | kernel: %.2f us | total: %.2f us\\n",
               memcpy_ms * 1000.0f, kernel_ms * 1000.0f, (memcpy_ms + kernel_ms) * 1000.0f);
    }

    void print_avg() {
        if (count == 0) return;
        printf("[nvfp4_group_gemm avg over %d calls] cudaMemcpy: %.2f us | kernel: %.2f us | total: %.2f us\\n",
               count,
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    if (!allocated) {
        cudaMallocHost(&h_groups, MAX_G * sizeof(GroupDesc));
...
#endif
    cudaMemcpyAsync(d_groups, h_groups, G * sizeof(GroupDesc), cudaMemcpyHostToDevice, 0);
#if TIMING_DEBUG
    ...
    if (!allocated) {
        cudaMalloc(&d_tmaps, 3 * NUM_CTAS * sizeof(CUtensorMap));
        allocated = true;
    }
    ...
    // === KERNEL LAUNCH ===
    auto kernel_inst_single = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE, PIPE_STAGES, NUM_WARPS, OUT_N_CHUNK, true>;
    auto kernel_inst_mult = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE, PIPE_STAGES, NUM_WARPS, OUT_N_CHUNK, false>;
    ...
    if (NUM_CTAS <= 148) {
        kernel_inst_single<<<NUM_CTAS, threads>>>(d_groups, tmap_a_temp, tmap_b_temp, tmap_c_temp, total_tiles, d_tmaps, N, K, G);
    } else {
        kernel_inst_mult<<<NUM_CTAS, threads>>>(d_groups, tmap_a_temp, tmap_b_temp, tmap_c_temp, total_tiles, d_tmaps, N, K, G);
    }
    ...
    timing_stats.record(memcpy_ms, kernel_ms);
    timing_stats.print_current(memcpy_ms, kernel_ms);
