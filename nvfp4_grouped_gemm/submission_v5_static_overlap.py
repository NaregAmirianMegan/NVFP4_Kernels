...
__global__ void __cluster_dims__(1, 1, 1) nvfp4_group_gemm_kernel(GroupDesc* groups, const __grid_constant__ CUtensorMap tmap_a_temp, 
    ...
    */
    ...
    __shared__ alignas(8) int64_t mbars[PIPE_STAGES * 2 + 3];
    ...
    const int mbar_addr_epi_done = mbar_addr_epi + 8;
    const int mbar_addr_debug = mbar_addr_epi_done + 8;

    int epi_phase = 0, epi_done_phase = 0, debug_phase = 0;

    if (warp_id == 0 && elect_one_sync()) {
        for (int i = 0; i < PIPE_STAGES * 2 + 3; i++) {
    ...
    for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
        ...
        // All warps aside from the two for TMA/MMA are dedicated to the epilogue
            ...
            mbar_wait(mbar_addr_epi, epi_phase);
            epi_phase ^= 1;
        ...
        if (warp_id == TMA_WARP || warp_id == MMA_WARP) {
            // TMA/MMA loop done for this tile
            asm volatile("bar.sync 3, %0;" :: "r"(WARP_SIZE * 2) : "memory");

            // Reinitialize mbarriers for next tile
            if (warp_id == TMA_WARP && elect_one_sync()) {
                for (int i = 0; i < PIPE_STAGES * 2; i++) {
                    mbar_init(mbar_addr_tma + i * 8, 1);
                }
        ...
        if (warp_id == MMA_WARP) {
            // MMA waits for EPI to complete before proceeding to next tile
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    kernel_inst<<<NUM_CTAS, threads>>>(d_groups, tmap_a_temp, tmap_b_temp, tmap_c_temp, total_tiles, d_tmaps, N, K, G);
#if TIMING_DEBUG
