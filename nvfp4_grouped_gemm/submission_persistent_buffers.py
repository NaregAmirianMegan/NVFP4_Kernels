...
"""
Persistent kernel with pre-allocated buffers
...
4. Removed cudaDeviceSynchronize() and cudaFree() from the hot path
...
__global__ void nvfp4_group_gemm_kernel(__half** __restrict__ c_ref, CUtensorMap* A_tmaps, CUtensorMap* B_tmaps,
                                        const uint8_t** sfa_ptrs, const uint8_t** sfb_ptrs, const int* __restrict__ M_sizes,
                                        const int* __restrict__ N_sizes, const int* __restrict__ K_sizes, const int* __restrict__ block_totals, int G) {
    /*
    ...
    for (int tile_idx = blockIdx.x; tile_idx < block_totals[G-1]; tile_idx += gridDim.x) {
        ...
        if (warp_id == TMA_WARP && elect_one_sync()) {
            auto tma_load_stage = [&](const int k_off, const int stage) {
                ...
                const uint8_t* sfa_g_ptr = sfa_gmem_base + ((k_off / 64) + (m_off / 128) * (K_sizes[group] / 64)) * 512;
                const uint8_t* sfb_g_ptr = sfb_gmem_base + ((k_off / 64) + (n_off / 128) * (K_sizes[group] / 64)) * 512;
                ...
                if constexpr (TD_MMA_N == 256) {
                    tcgen05_1dtma_g2s_sf(sfb_smem_stage_ptr+SF_SMEM_TILESZ, sfb_g_ptr + (K_sizes[group] / 64)*512, SF_SMEM_TILESZ, mbar_addr_tma_stage, CacheHintSm100::EVICT_NORMAL);
                }
            ...
            int stage = 0;
            for (int k_off = TD_SMEM_K * PIPE_STAGES; k_off < K_sizes[group]; k_off += TD_SMEM_K) {
            ...
            for (int k_off = 0; k_off < K_sizes[group]; k_off += TD_SMEM_K) {
            ...
            for (int sub_m = 0; sub_m < rows_per_warp / 16; sub_m++) {
                ...
                for (int i = 0; i < TD_MMA_N / 8; i++) {
                    ...
                    if (m_offset < M_sizes[group]) {
                        reinterpret_cast<half2 *>(c_ref[group] + (m_offset)*N_sizes[group] + n_offset)[0] = __float22half2_rn({results[i * 4], results[i * 4 + 1]});
                    }
                    if (m_offset + 8 < M_sizes[group]) {
                        reinterpret_cast<half2 *>(c_ref[group] + (m_offset + 8)*N_sizes[group] + n_offset)[0] = __float22half2_rn({results[i * 4 + 2], results[i * 4 + 3]});
                    }
                }
            }
        }
...
/*
 * This struct holds all the buffers we need for the kernel launch.
 * Instead of allocating/freeing on every call, we keep them around.
 *
 * KEY INSIGHT: cudaMalloc and cudaFree are expensive (~10-50 microseconds each).
 * By reusing buffers, we only pay this cost once (or when we need to grow).
 */
struct PersistentBuffers {
    // ========== DEVICE BUFFERS ==========
    // These live on the GPU and are passed to the kernel
    CUtensorMap* d_A_tmaps = nullptr;      // TMA descriptors for A matrices
    CUtensorMap* d_B_tmaps = nullptr;      // TMA descriptors for B matrices
    int* d_block_totals = nullptr;         // Cumulative block counts per group
    int* d_M_data = nullptr;               // M dimensions per group
    int* d_N_data = nullptr;               // N dimensions per group
    int* d_K_data = nullptr;               // K dimensions per group

    // ========== HOST BUFFERS (PINNED) ==========
    // These live in pinned (page-locked) host memory.
    // Pinned memory enables faster Host->Device transfers because:
    // 1. DMA can access it directly without copying through staging buffers
    // 2. It can be used with cudaMemcpyAsync for overlapping transfers
    CUtensorMap* h_A_tmaps = nullptr;      // TMA descriptors built on host
    CUtensorMap* h_B_tmaps = nullptr;      // TMA descriptors built on host
    int* h_block_totals = nullptr;         // Block totals computed on host

    // ========== CAPACITY TRACKING ==========
    int capacity = 0;  // Current max number of groups we can handle

    // ========== CACHED DRIVER FUNCTION ==========
    // The driver function lookup is also expensive, so cache it
    PFN_cuTensorMapEncodeTiled_v12000 cuTensorMapEncodeTiled = nullptr;

    /*
     * Ensure we have enough buffer space for G groups.
     * Only reallocates if current capacity is insufficient.
     * Allocates 2x the requested size as headroom to avoid frequent reallocs.
     */
    void ensure_capacity(int G) {
        if (G <= capacity) {
            return;  // Already have enough space - this is the fast path!
        }

        // Need to grow - free old buffers first
        if (d_A_tmaps) cudaFree(d_A_tmaps);
        if (d_B_tmaps) cudaFree(d_B_tmaps);
        if (d_block_totals) cudaFree(d_block_totals);
        if (d_M_data) cudaFree(d_M_data);
        if (d_N_data) cudaFree(d_N_data);
        if (d_K_data) cudaFree(d_K_data);

        if (h_A_tmaps) cudaFreeHost(h_A_tmaps);
        if (h_B_tmaps) cudaFreeHost(h_B_tmaps);
        if (h_block_totals) cudaFreeHost(h_block_totals);

        // Allocate with 2x headroom to reduce future reallocations
        int new_capacity = G * 2;

        // Device allocations
        cudaMalloc(&d_A_tmaps, new_capacity * sizeof(CUtensorMap));
        cudaMalloc(&d_B_tmaps, new_capacity * sizeof(CUtensorMap));
        cudaMalloc(&d_block_totals, new_capacity * sizeof(int));
        cudaMalloc(&d_M_data, new_capacity * sizeof(int));
        cudaMalloc(&d_N_data, new_capacity * sizeof(int));
        cudaMalloc(&d_K_data, new_capacity * sizeof(int));

        // Host allocations using cudaMallocHost for pinned memory
        cudaMallocHost(&h_A_tmaps, new_capacity * sizeof(CUtensorMap));
        cudaMallocHost(&h_B_tmaps, new_capacity * sizeof(CUtensorMap));
        cudaMallocHost(&h_block_totals, new_capacity * sizeof(int));

        capacity = new_capacity;
    }

    /*
     * Initialize the driver function pointer (only done once)
     */
    void init_driver_function() {
        if (cuTensorMapEncodeTiled == nullptr) {
            cuTensorMapEncodeTiled = get_cuTensorMapEncodeTiled();
        }
    }
};
...
// ============================================================================
// This is the key: a static global that persists across function calls.
// First call: allocates buffers
// Subsequent calls: reuses existing buffers (unless G grows)
static PersistentBuffers g_buffers;

torch::Tensor nvfp4_group_gemm(torch::Tensor A_ptrs, torch::Tensor B_ptrs, torch::Tensor C_ptrs,
                               torch::Tensor SFA_ptrs, torch::Tensor SFB_ptrs,
                               torch::Tensor M_sizes, torch::Tensor N_sizes, torch::Tensor K_sizes, int G) {

    // ========== STEP 1: One-time initialization ==========
    // These only do work on the first call
    g_buffers.init_driver_function();  // Cache driver function pointer
    g_buffers.ensure_capacity(G);       // Allocate buffers if needed
    ...
    constexpr CUtensorMapSwizzle SWIZZLE_TYPE = SWIZZLE ? CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B
    ...
    // ========== STEP 2: Build TMA descriptors ==========
    // This still runs every call because the tensor addresses change.
    // But we write to pre-allocated pinned host memory.
    for (int i = 0; i < G; ++i) {
        g_buffers.h_block_totals[i] = CEIL_DIV(M_data[i], M_TILE_SIZE) * CEIL_DIV(N_data[i], N_TILE_SIZE)
                                    + (i > 0 ? g_buffers.h_block_totals[i-1] : 0);

        tma_3d_map_ab<M_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(
            g_buffers.cuTensorMapEncodeTiled,
            &g_buffers.h_A_tmaps[i],
            reinterpret_cast<void*>(A_ptrs_data[i]),
            M_data[i], K_data[i]);

        tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(
            g_buffers.cuTensorMapEncodeTiled,
            &g_buffers.h_B_tmaps[i],
            reinterpret_cast<void*>(B_ptrs_data[i]),
            N_data[i], K_data[i]);
    }

    // ========== STEP 3: Copy to device ==========
    // Using pre-allocated device buffers (no cudaMalloc here!)
    cudaMemcpy(g_buffers.d_A_tmaps, g_buffers.h_A_tmaps, G * sizeof(CUtensorMap), cudaMemcpyHostToDevice);
    cudaMemcpy(g_buffers.d_B_tmaps, g_buffers.h_B_tmaps, G * sizeof(CUtensorMap), cudaMemcpyHostToDevice);
    cudaMemcpy(g_buffers.d_block_totals, g_buffers.h_block_totals, G * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(g_buffers.d_M_data, M_data, G * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(g_buffers.d_N_data, N_data, G * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(g_buffers.d_K_data, K_data, G * sizeof(int), cudaMemcpyHostToDevice);

    // ========== STEP 4: Launch kernel ==========
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE,
                                               M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE, PIPE_STAGES, NUM_WARPS>;
    ...
    kernel_inst<<<grid_dim, threads>>>(
        reinterpret_cast<__half**>(C_ptrs.data_ptr()),
        g_buffers.d_A_tmaps, g_buffers.d_B_tmaps,
        reinterpret_cast<const uint8_t**>(SFA_ptrs.data_ptr()),
        reinterpret_cast<const uint8_t**>(SFB_ptrs.data_ptr()),
        g_buffers.d_M_data, g_buffers.d_N_data, g_buffers.d_K_data,
        g_buffers.d_block_totals, G);

    // ========== KEY DIFFERENCE: No cleanup! ==========
    // REMOVED: cudaDeviceSynchronize() - let kernel run async
    // REMOVED: cudaFree() calls - buffers persist for next call
...
torch::Tensor nvfp4_group_gemm(torch::Tensor A_ptrs, torch::Tensor B_ptrs, torch::Tensor C_ptrs, torch::Tensor SFA_ptrs, torch::Tensor SFB_ptrs, torch::Tensor M_sizes, torch::Tensor N_sizes, torch::Tensor K_sizes, int G);

"""


nvfp4_group_gemm_module = load_inline(
    name='nvfp4_group_gemm_persistent',
...
def custom_kernel(data: input_t) -> output_t:
    ...
    nvfp4_group_gemm_module.nvfp4_group_gemm(A_ptrs, B_ptrs, C_ptrs, SFA_ptrs, SFB_ptrs, M_sizes, N_sizes, K_sizes, G)
