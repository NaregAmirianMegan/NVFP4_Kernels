...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int PIPE_STAGES, int NUM_WARPS, int OUT_CHUNK, bool TRANSPOSE>
__global__ void __cluster_dims__(1, 1, 1) nvfp4_group_gemm_kernel(const __grid_constant__ GroupDescs group_descs, const __grid_constant__ CUtensorMap tmap_a_temp,
    ...
    */
    ...
    CUtensorMap* g_A_tmap = d_tmaps + 3*blockIdx.x;
    CUtensorMap* g_B_tmap = g_A_tmap + 1;
    CUtensorMap* g_C_tmap = g_B_tmap + 1;
    ...
    */
    ...
    __shared__ alignas(8) int64_t mbars[PIPE_STAGES * 2 + 4];
    ...
    for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
        ...
        // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops
            ...
            for (int k_off = 0; k_off < K; k_off += TD_SMEM_K) {
                ...
                for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                    ...
                    // Switch inputs to MMA instruction if computing transpose internally
                    // Inputting k_off like this will set enable-input-d so only on the first mma we 0 out the result space in TMEM
                    if constexpr(TRANSPOSE) {
                        tcgen05_mma_nvfp4<1>(tmem_addr_result, b_desc, a_desc, make_instr_desc<TD_MMA_N, TD_MMA_M>(), sfb_tmem, sfa_tmem, k_off + sub_k_iter);
                    } else {
        ...
        // All warps aside from the two for TMA/MMA are dedicated to the epilogue
            ...
            if (update_group && warp_id == 0) {
                if (elect_one_sync()) {
                    if constexpr (TRANSPOSE) {
                        tmap_update<0>(local_C_tmap_addr, groups[group].C_addr, groups[group].M, groups[group].M * sizeof(__half)); 
                    } else {
        ...
        // TMA OVERLAP EPILOGUE
        
            constexpr int res_per_thrd = 4 * (OUT_CHUNK / 8);
            float results[res_per_thrd];
            constexpr int rows_per_warp = (TRANSPOSE ? TD_SMEM_N : TD_SMEM_M) / (NUM_WARPS - 2);
            ...
            for (int chunk = 0; chunk < NUM_CHUNKS; chunk++) {
                ...
                if constexpr (TRANSPOSE) {
                    for (int sub_n = 0; sub_n < rows_per_warp / 16; sub_n++) {
                        tcgen05_ld<16, 256, OUT_CHUNK / 8>(results, tmem_addr_result + (((warp_id * rows_per_warp) + sub_n * 16) << 16) + chunk * OUT_CHUNK);
                        ...
                        for (int i = 0; i < OUT_CHUNK / 8; i++) {
                            ...
                            if (m_offset + m_off < groups[group].M) {
                                reinterpret_cast<half2 *>(c_smem + (C_CHUNK_SMEM_TILESZ * buf) + n_offset*OUT_CHUNK + m_offset)[0] = __float22half2_rn({results[i * 4], results[i * 4 + 1]});
                                reinterpret_cast<half2 *>(c_smem + (C_CHUNK_SMEM_TILESZ * buf) + (n_offset + 8)*OUT_CHUNK + m_offset)[0] = __float22half2_rn({results[i * 4 + 2], results[i * 4 + 3]});
                            }
                        }
                    }
                } else {
                ...
                if (warp_id == 0 && elect_one_sync()) {
                    ...
                    if constexpr (TRANSPOSE) {
                        tcgen05_2dtma_s2g_c(c_smem_ptr + (C_CHUNK_SMEM_TILESZ * 2 * buf), g_C_tmap, n_off, m_off + (chunk * OUT_CHUNK), CacheHintSm100::EVICT_NORMAL);
                    } else {
                        tcgen05_2dtma_s2g_c(c_smem_ptr + (C_CHUNK_SMEM_TILESZ * 2 * buf), g_C_tmap, m_off, n_off + (chunk * OUT_CHUNK), CacheHintSm100::EVICT_NORMAL);
                    }
        ...
        /*
            ...
            for (int sub_m = 0; sub_m < rows_per_warp / 16; sub_m++) {
                ...
                for (int i = 0; i < TD_MMA_N / 8; i++) {
                    ...
                    const int n_offset = i * 8 + (lane_id % 4) * 2;
                    if (m_offset + m_off < groups[group].M) {
                        reinterpret_cast<half2 *>(c_smem + m_offset*TD_SMEM_N + n_offset)[0] = __float22half2_rn({results[i * 4], results[i * 4 + 1]});
                    }
                    if (m_offset + m_off + 8 < groups[group].M) {
                        reinterpret_cast<half2 *>(c_smem + (m_offset + 8)*TD_SMEM_N + n_offset)[0] = __float22half2_rn({results[i * 4 + 2], results[i * 4 + 3]});
                    }
                }
            }
            ...
            if (warp_id == 0 && elect_one_sync()) {
                ...
                asm volatile("cp.async.bulk.commit_group;");
                asm volatile("cp.async.bulk.wait_group.read 0;"); // ISSUE: add .read or no?
                mbar_arrive(mbar_addr_epi_done, 1);
            }
            asm volatile("bar.sync 2, %0;" :: "r"(WARP_SIZE * (NUM_WARPS - 2)) : "memory");
        */
    ...
    // Free memory
    __syncthreads();
    if (warp_id == 0) { tcgen05_dealloc_tmem<1>(tmem_addr_base, TMEM_RES_COLS * 2 * 2); }
}
...
std::vector<torch::Tensor> nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    if (!allocated) {
        cudaMalloc(&d_tmaps, 3 * 148 * sizeof(CUtensorMap));
        allocated = true;
    }
    ...
    if (N != cache_N || K != cache_K) {
        ...
        if constexpr (TRANSPOSE) {
            tma_2d_map_c_init<N_TILE_SIZE, OUT_CHUNK>(cuTensorMapEncodeTiled_fn, &tmap_c_temp, nullptr, N, M_TILE_SIZE);
        } else {
    ...
    // === KERNEL LAUNCH ===
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE_TYPE, PIPE_STAGES, NUM_WARPS, OUT_CHUNK, TRANSPOSE>;
    ...
    kernel_inst<<<NUM_CTAS, threads>>>(gd, tmap_a_temp, tmap_b_temp, tmap_c_temp, total_tiles, d_tmaps, N, K, G);

    std::vector<torch::Tensor> outputs;
    outputs.reserve(G);
    if (TRANSPOSE) {
        for (int i = 0; i < G; i++) {
            outputs.push_back(std::get<2>(abc_tensors[i]).view({N, std::get<0>(prob_sizes[i]), 1}).transpose(0, 1));
        }
    }
