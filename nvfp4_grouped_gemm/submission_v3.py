    ...
    for (int tile_idx = blockIdx.x; tile_idx < block_totals[G-1]; tile_idx += gridDim.x) {
        ...
        if (warp_id == TMA_WARP && elect_one_sync()) {
            auto tma_load_stage = [&](const int k_off, const int stage) {
                ...
                */
                const uint8_t* sfa_g_ptr = sfa_gmem_base + ((k_off / 64) + (m_off / 128) * (K_sizes[group] / 64)) * 512; // ISSUE: These could be just simple bit shifts, adjust if compiler doesn't
...
torch::Tensor nvfp4_group_gemm(torch::Tensor A_ptrs, torch::Tensor B_ptrs, torch::Tensor C_ptrs, torch::Tensor SFA_ptrs, torch::Tensor SFB_ptrs, torch::Tensor M_sizes, torch::Tensor N_sizes, torch::Tensor K_sizes, int G) { 
    ...
    kernel_inst<<<grid_dim, threads>>>(reinterpret_cast<__half**>(C_ptrs.data_ptr()), d_A_tmaps, d_B_tmaps, reinterpret_cast<const uint8_t**>(SFA_ptrs.data_ptr()), reinterpret_cast<const uint8_t**>(SFB_ptrs.data_ptr()), 
                                        d_M_data, d_N_data, d_K_data, d_block_totals, G);
