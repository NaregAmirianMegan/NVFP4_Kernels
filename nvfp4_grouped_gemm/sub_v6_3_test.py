    ...
    */
    ...
    for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
        ...
        // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops
            ...
            for (int k_off = 0; k_off < K; k_off += TD_SMEM_K) {
                ...
                for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                    ...
                    int sfa_tmem = tmem_addr_sfa + 4 * sub_k_iter + (m_off%128)/32;
                    int sfb_tmem;
                    if constexpr (TD_MMA_N == 256) {
        ...
        // All warps aside from the two for TMA/MMA are dedicated to the epilogue
            ...
            constexpr int res_per_thrd = TRANSPOSE ? OUT_CHUNK : 4 * (OUT_CHUNK / 8);
            float results[res_per_thrd];
            constexpr int rows_per_warp = (TRANSPOSE ? TD_SMEM_N : TD_SMEM_M) / (NUM_WARPS - 2);
            constexpr int NUM_CHUNKS = (TRANSPOSE ? TD_MMA_M : TD_MMA_N) / OUT_CHUNK;
            for (int chunk = 0; chunk < NUM_CHUNKS; chunk++) {
                ...
                if constexpr (TRANSPOSE) {
                    tcgen05_ld<32, 32, OUT_CHUNK>(results, tmem_addr_result + ((warp_id * 32) << 16) + chunk * OUT_CHUNK); // Each thread in a warp loads OUT_CHUNK values along M for a col along N
                    ...
                    for (int i = 0; i < OUT_CHUNK; i++) {
                        if (m_off + chunk * OUT_CHUNK + i >= groups[group].M) break;
                        reinterpret_cast<half *>(c_smem + (C_CHUNK_SMEM_TILESZ * buf) + i*TD_SMEM_N + threadIdx.x)[0] = __float2half_rn(results[i]);
                    }
                } else {
                ...
                if (warp_id == 0 && elect_one_sync()) {
                    ...
                    tcgen05_2dtma_s2g_c(c_smem_ptr + (C_CHUNK_SMEM_TILESZ * 2 * buf), g_C_tmap, m_off + (chunk * OUT_CHUNK), n_off, CacheHintSm100::EVICT_NORMAL);
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    int NUM_CTAS = total_tiles < 148*CTAS_PER_SM ? total_tiles : 148*CTAS_PER_SM;

    if (!allocated) {
        cudaMalloc(&d_tmaps, 3 * 148*CTAS_PER_SM * sizeof(CUtensorMap));
        allocated = true;
    }
