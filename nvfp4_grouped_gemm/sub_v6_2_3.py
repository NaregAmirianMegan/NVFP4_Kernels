...
__device__ inline void tmap_fence_proxy(const CUtensorMap* g_tmap, const int local_tmap_addr) {
    asm volatile(
        "tensormap.cp_fenceproxy.global.shared::cta.tensormap::generic.release.cta.sync.aligned [%0], [%1], 128;"
        :
...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int CLUSTER_SIZE>
__global__ void __launch_bounds__(WARP_SIZE * NUM_WARPS) __cluster_dims__(CLUSTER_SIZE, 1, 1) nvfp4_group_gemm_kernel(const __grid_constant__ GroupDescs group_descs,
                                                                                                                      const int total_tiles, const int N, const int K, 
                                                                                                                      const int G) {
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    for (int i = 0; i < G; i++) {
        ...
        tma_3d_map_ab<M_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled_fn, &gd.groups[i].tmap_a, std::get<0>(abc_tensors[i]).data_ptr(), M_size, K);
        tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled_fn, &gd.groups[i].tmap_b, std::get<1>(abc_tensors[i]).data_ptr(), N, K);
        tma_2d_map_c_init<M_TILE_SIZE, OUT_N_CHUNK>(cuTensorMapEncodeTiled_fn, &gd.groups[i].tmap_c, reinterpret_cast<__half*>(std::get<2>(abc_tensors[i]).data_ptr()), M_size, N);
    ...
    // === KERNEL LAUNCH ===
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE_TYPE, PIPE_STAGES, NUM_WARPS, OUT_N_CHUNK, TILE_DESC_PIPE_STAGES, CLUSTER_SIZE>;
    ...
    kernel_inst<<<NUM_CTAS, threads>>>(gd, total_tiles, N, K, G);
