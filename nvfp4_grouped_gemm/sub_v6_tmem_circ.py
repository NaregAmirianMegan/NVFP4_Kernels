...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int TD_MMA_M, int TD_MMA_N, int TD_MMA_K, CUtensorMapSwizzle SWIZZLE_TYPE, int PIPE_STAGES, int NUM_WARPS, int OUT_N_CHUNK>
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    // === KERNEL LAUNCH ===
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE_TYPE, PIPE_STAGES, NUM_WARPS, OUT_N_CHUNK>;
