...
"""
Try overlapping MMA with epilogue via ping-pong TMEM buffers
"""
...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int PIPE_STAGES, int NUM_WARPS, int OUT_N_CHUNK, int SPLIT_K>
__global__ void __cluster_dims__(1, 1, 1) nvfp4_group_gemm_kernel(const __grid_constant__ GroupDescs group_descs, const __grid_constant__ CUtensorMap tmap_a_temp,
    ...
    */
    ...
    __shared__ alignas(8) int64_t mbars[PIPE_STAGES * 2 + 8];
    ...
    const int mbar_addr_epi_done = mbar_addr_epi + 4 * 8;

    int epi_phase[4] = {0, 0, 0, 0};
    int epi_done_phase[4] = {0, 0, 0, 0};

    int tmem_buf = 0;

    if (warp_id == 0 && elect_one_sync()) {
        for (int i = 0; i < PIPE_STAGES * 2 + 8; i++) {
            mbar_init(mbar_addr_tma + i * 8, 1);
        }
        asm volatile("fence.mbarrier_init.release.cluster;"); // ISSUE: Verify we need this here
    } else if (warp_id == 1) { 
        tcgen05_alloc_tmem<1>(tmem_addr_ptr, TD_MMA_N*2*4); 
    }
    __syncthreads(); // Ensure all threads have correct TMEM ptrs

    const int tmem_addr_base = tmem_addr_ptr[0];
    const int tmem_result_ptrs[4] = {tmem_addr_base, tmem_addr_base + 2*TD_MMA_N, tmem_addr_base + 4*TD_MMA_N, tmem_addr_base + 6*TD_MMA_N};
    const int tmem_sfa_ptrs[4] = {tmem_result_ptrs[0] + TD_MMA_N, tmem_result_ptrs[1] + TD_MMA_N, tmem_result_ptrs[2] + TD_MMA_N, tmem_result_ptrs[3] + TD_MMA_N};
    const int tmem_sfb_ptrs[4] = {tmem_sfa_ptrs[0] + (TD_MMA_M/32) * (TD_SMEM_K/64), tmem_sfa_ptrs[1] + (TD_MMA_M/32) * (TD_SMEM_K/64), tmem_sfa_ptrs[2] + (TD_MMA_M/32) * (TD_SMEM_K/64), tmem_sfa_ptrs[3] + (TD_MMA_M/32) * (TD_SMEM_K/64)};
    ...
    int init_tmem_fill = 0;

    for (int tile_idx_splitk = blockIdx.x; tile_idx_splitk < total_tiles * SPLIT_K; tile_idx_splitk += gridDim.x) {
        const int tile_idx = tile_idx_splitk / SPLIT_K;
        const int split_k_idx = tile_idx_splitk % SPLIT_K;
        const int k_split_off = split_k_idx * (K / SPLIT_K);
        ...
        if (warp_id == TMA_WARP) {
            ...
            if (elect_one_sync()) {
                ...
                if (tile_idx_splitk == blockIdx.x) {
                    const int k_tiles = (K / SPLIT_K) / TD_SMEM_K;
                    const int prefetch_stages = PIPE_STAGES > k_tiles ? k_tiles : PIPE_STAGES;
                    for (int stage = 0; stage < prefetch_stages; stage++) {
                        tma_load_stage(stage * TD_SMEM_K + k_split_off, stage);
                    }
        ...
        // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops
            ...
            for (int k_off = 0; k_off < K / SPLIT_K; k_off += TD_SMEM_K) {
                const int stage = (glob_k_off / TD_SMEM_K) % PIPE_STAGES;
        ...
        /*
            ...
            for (int chunk = 0; chunk < TD_MMA_N / OUT_N_CHUNK; chunk++) {
                ...
                if (warp_id == 0 && elect_one_sync()) {
                    ...
                    tcgen05_2dtma_s2g_c(c_smem_ptr + (C_CHUNK_SMEM_TILESZ * 2 * buf), g_C_tmap, m_off, n_off + (chunk * OUT_N_CHUNK), CacheHintSm100::EVICT_NORMAL);
            ...
            for (int sub_m = 0; sub_m < rows_per_warp / 16; sub_m++) {
                ...
                for (int i = 0; i < TD_MMA_N / 8; i++) {
                    ...
                    if (m_offset < groups[group].M) {
                        ...
                        atomicAdd(reinterpret_cast<half2 *>(groups[group].C_addr + (m_offset)*N + n_offset), __float22half2_rn({results[i * 4], results[i * 4 + 1]}));
                    }
                    if (m_offset + 8 < groups[group].M) {
                        ...
                        atomicAdd(reinterpret_cast<half2 *>(groups[group].C_addr + (m_offset + 8)*N + n_offset), __float22half2_rn({results[i * 4 + 2], results[i * 4 + 3]}));
                    }
                }
            }
            asm volatile("bar.sync 2, %0;" :: "r"(WARP_SIZE * (NUM_WARPS - 2)) : "memory");
    ...
    // Free memory
    __syncthreads();
    if (warp_id == 0) { tcgen05_dealloc_tmem<1>(tmem_addr_base, TD_MMA_N * 2 * 4); }
}
...
static __half* c_buffer;
static size_t c_buffer_offset = 0;

constexpr size_t C_BUFFER_BYTES = 2048ull * 1024 * 1024;
...
std::vector<torch::Tensor> nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    constexpr int PIPE_STAGES = 15;
    constexpr int OUT_N_CHUNK = 32;
    constexpr int SPLIT_K = 4;

    constexpr CUtensorMapSwizzle SWIZZLE_TYPE = CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_32B;

    if (!allocated) {
        cudaMalloc(&c_buffer, C_BUFFER_BYTES);
        cudaMemset(c_buffer, 0, C_BUFFER_BYTES);
        c_buffer_offset = 0;
    }
    ...
    for (int i = 0; i < G; i++) {
        ...
        __half* c_ptr = reinterpret_cast<__half*>(reinterpret_cast<char*>(c_buffer) + c_buffer_offset);
        gd.groups[i].C_addr     = c_ptr;
        ...
        const int M_size = std::get<0>(prob_sizes[i]);
        gd.groups[i].M          = M_size;
        gd.groups[i].block_start = total_tiles;
        total_tiles += CEIL_DIV(M_size, M_TILE_SIZE) * N_blocks;

        outputs.push_back(torch::from_blob(
            c_ptr,
            {static_cast<int64_t>(M_size), static_cast<int64_t>(N), 1},
            noop_deleter,
            torch::TensorOptions().dtype(torch::kFloat16).device(std::get<2>(abc_tensors[i]).device())
        ));
        c_buffer_offset += static_cast<size_t>(M_size) * static_cast<size_t>(N) * sizeof(__half);
    }
    ...
    // === KERNEL LAUNCH ===
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE_TYPE, PIPE_STAGES, NUM_WARPS, OUT_N_CHUNK, SPLIT_K>;
...
def custom_kernel(data: input_t) -> output_t:
    ...
    return nvfp4_group_gemm_module.nvfp4_group_gemm(
        abc_tensors, sfasfb_tensors_reordered, problem_sizes, N, K, G
    )
