...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    for (int i = 0; i < G; i++) {
        ...
        tma_3d_map_ab<M_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled_fn, &gd.groups[i].tmap_a, std::get<0>(abc_tensors[i]).data_ptr(), M_size, K_size);
        tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled_fn, &gd.groups[i].tmap_b, std::get<1>(abc_tensors[i]).data_ptr(), N_size, K_size);
        tma_2d_map_c_init<M_TILE_SIZE, OUT_N_CHUNK>(cuTensorMapEncodeTiled_fn, &gd.groups[i].tmap_c, reinterpret_cast<__half*>(std::get<2>(abc_tensors[i]).data_ptr()), M_size, N_size);
