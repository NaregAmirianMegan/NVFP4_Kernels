...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int PIPE_STAGES, int NUM_WARPS, int OUT_CHUNK, int TILE_DESC_PIPE_STAGES, bool TRANSPOSE>
        ...
        if (warp_id == TMA_WARP) {
            if (elect_one_sync()) {
                auto tma_load_stage = [&](const int k_off, const int stage) {
                    ...
                    tcgen05_3dtma_g2s_ab<1>(a_smem_stage_ptr, g_A_tmap + tile_stage, m_off, k_off_coremat, mbar_addr_tma_stage, CacheHintSm100::EVICT_NORMAL);
...
std::vector<torch::Tensor> nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, 
                                                K_MMA_SIZE, SWIZZLE_TYPE, PIPE_STAGES, NUM_WARPS, OUT_CHUNK, TILE_DESC_PIPE_STAGES, TRANSPOSE>;
