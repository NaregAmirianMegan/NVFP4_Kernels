...
"""
Reduced overhead - WITH TIMING INSTRUMENTATION
"""
...
struct PersistentState {
    // Pinned + mapped memory: write on host, read on device
    GroupDesc* h_groups;    // Host-accessible pointer
    GroupDesc* d_groups;    // Device-accessible pointer (same physical memory)

    size_t max_G;
};

// Timing structure to hold all measurements
struct TimingInfo {
    double tma_encode_us;
    double host_alloc_us;
    double device_alloc_us;
    double data_prep_us;
    double h2d_us;
    double func_attr_us;
    double kernel_launch_us;
    double kernel_exec_us;
    double host_dealloc_us;
    double device_dealloc_us;
    double total_us;
};
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    // === TMA ENCODE ===
    auto tma_start = std::chrono::high_resolution_clock::now();
    ...
    auto tma_end = std::chrono::high_resolution_clock::now();
    timing.tma_encode_us = std::chrono::duration<double, std::micro>(tma_end - tma_start).count();
    timing.total_us += timing.tma_encode_us;
    ...
    timing.host_alloc_us = std::chrono::duration<double, std::micro>(host_alloc_end - host_alloc_start).count();
    timing.total_us += timing.host_alloc_us;
    ...
    timing.device_alloc_us = std::chrono::duration<double, std::micro>(device_alloc_end - device_alloc_start).count();
    timing.total_us += timing.device_alloc_us;
    ...
    timing.data_prep_us = std::chrono::duration<double, std::micro>(data_prep_end - data_prep_start).count();
    timing.total_us += timing.data_prep_us;
    ...
    timing.h2d_us = h2d_ms * 1000;
    timing.total_us += timing.h2d_us;

    // === KERNEL SETUP (cudaFuncSetAttribute) ===
    auto func_attr_start = std::chrono::high_resolution_clock::now();
    ...
    auto func_attr_end = std::chrono::high_resolution_clock::now();
    timing.func_attr_us = std::chrono::duration<double, std::micro>(func_attr_end - func_attr_start).count();
    timing.total_us += timing.func_attr_us;

    // === KERNEL LAUNCH ===
    auto kernel_launch_start = std::chrono::high_resolution_clock::now();

    constexpr int threads = WARP_SIZE * NUM_WARPS;

    // Record start event for kernel execution timing
    cudaEventRecord(start);

    kernel_inst<<<NUM_CTAS, threads>>>(d_groups, tmap_a_temp, tmap_b_temp, total_tiles, d_tmaps, N, K, G);

    auto kernel_launch_end = std::chrono::high_resolution_clock::now();
    timing.kernel_launch_us = std::chrono::duration<double, std::micro>(kernel_launch_end - kernel_launch_start).count();
    timing.total_us += timing.kernel_launch_us;

    // === KERNEL EXECUTION (synchronize and measure) ===
    cudaEventRecord(stop);
    cudaEventSynchronize(stop);

    float kernel_exec_ms_gpu;
    cudaEventElapsedTime(&kernel_exec_ms_gpu, start, stop);
    timing.kernel_exec_us = kernel_exec_ms_gpu * 1000.0;  // Convert ms to us
    timing.total_us += timing.kernel_exec_us;

    // === PRINT TIMING REPORT ===
    printf("\\n========== NVFP4 GROUP GEMM TIMING REPORT ==========\\n");
    printf("Problem: G=%d, N=%d, K=%d, total_tiles=%d\\n", G, N, K, total_tiles);
    printf("----------------------------------------------------\\n");
    printf("TMA Encode:           %10.2f us\\n", timing.tma_encode_us);
    printf("Host Allocation:      %10.2f us\\n", timing.host_alloc_us);
    printf("Device Allocation:    %10.2f us\\n", timing.device_alloc_us);
    printf("Data Preparation:     %10.2f us\\n", timing.data_prep_us);
    printf("H2D Copy:             %10.2f us\\n", timing.h2d_us);
    printf("Func Attribute Set:   %10.2f us\\n", timing.func_attr_us);
    printf("Kernel Launch:        %10.2f us\\n", timing.kernel_launch_us);
    printf("Kernel Execution:     %10.2f us (GPU time)\\n", timing.kernel_exec_us);
    printf("----------------------------------------------------\\n");
    printf("Total Time:           %10.2f us\\n", timing.total_us);
    printf("Overhead (non-kernel): %10.2f us (%.2f%%)\\n",
           timing.total_us - timing.kernel_exec_us,
           100.0 * (timing.total_us - timing.kernel_exec_us) / timing.total_us);
    printf("====================================================\\n\\n");
