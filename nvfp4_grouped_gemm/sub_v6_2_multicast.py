...
template<int CTA_GROUP, int CLUSTER_SIZE>
__device__ void inline tcgen05_3dtma_g2s_ab_multicast(int dst_smem, const void *tmap_ptr, int mn_off, int k_off_coremat, int mbar_addr, CacheHintSm100 cache_policy) {
    constexpr uint16_t CTA_MASK = ((uint16_t)1 << CLUSTER_SIZE) - 1; // Set first CLUSTER_SIZE bits to 1
    asm volatile (
        ...
        :
        : "n"(CTA_GROUP), "r"(dst_smem), "l"(tmap_ptr), "r"(0), "r"(mn_off), "r"(k_off_coremat), "r"(mbar_addr), "h"(CTA_MASK), "l"(cache_policy)
    );
}
...
template<int MN_SMEM_TD, int K_SMEM_TD, int CLUSTER_SIZE, CUtensorMapSwizzle SWIZZLE>
...
template<int MN_SMEM_TD, int K_SMEM_TD, int CLUSTER_SIZE>
struct tma_3d_map_ab<MN_SMEM_TD, K_SMEM_TD, CLUSTER_SIZE, CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_NONE> {
    static void init(PFN_cuTensorMapEncodeTiled_v12000 cuTensorMapEncodeTiled, CUtensorMap* tmap, void* ptr, uint64_t mn_dim_gmem, uint64_t k_dim_gmem) {
...
template<int MN_SMEM_TD, int K_SMEM_TD, int CLUSTER_SIZE>
struct tma_3d_map_ab<MN_SMEM_TD, K_SMEM_TD, CLUSTER_SIZE, CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_128B> {
    static void init(PFN_cuTensorMapEncodeTiled_v12000 cuTensorMapEncodeTiled, CUtensorMap* tmap, void* ptr, uint64_t mn_dim_gmem, uint64_t k_dim_gmem) {
        constexpr uint32_t rank = 3;
        uint64_t dim_gmem[rank] = {256, mn_dim_gmem, k_dim_gmem/256};
        uint64_t stride_gmem[rank - 1] = {k_dim_gmem/2, 128};
        uint32_t dim_smem[rank] = {256, MN_SMEM_TD / CLUSTER_SIZE, K_SMEM_TD/256};
        uint32_t elem_stride[rank] = {1, 1, 1};
...
template<int MN_SMEM_TD, int K_SMEM_TD, int CLUSTER_SIZE>
struct tma_3d_map_ab<MN_SMEM_TD, K_SMEM_TD, CLUSTER_SIZE, CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_64B> {
    static void init(PFN_cuTensorMapEncodeTiled_v12000 cuTensorMapEncodeTiled, CUtensorMap* tmap, void* ptr, uint64_t mn_dim_gmem, uint64_t k_dim_gmem) {
        constexpr uint32_t rank = 3;
        uint64_t dim_gmem[rank] = {128, mn_dim_gmem, k_dim_gmem/128};
        uint64_t stride_gmem[rank - 1] = {k_dim_gmem/2, 64};
        uint32_t dim_smem[rank] = {128, MN_SMEM_TD / CLUSTER_SIZE, K_SMEM_TD/128};
        uint32_t elem_stride[rank] = {1, 1, 1};
...
template<int MN_SMEM_TD, int K_SMEM_TD, int CLUSTER_SIZE>
struct tma_3d_map_ab<MN_SMEM_TD, K_SMEM_TD, CLUSTER_SIZE, CUtensorMapSwizzle::CU_TENSOR_MAP_SWIZZLE_32B> {
    static void init(PFN_cuTensorMapEncodeTiled_v12000 cuTensorMapEncodeTiled, CUtensorMap* tmap, void* ptr, uint64_t mn_dim_gmem, uint64_t k_dim_gmem) {
        constexpr uint32_t rank = 3;
        uint64_t dim_gmem[rank] = {64, mn_dim_gmem, k_dim_gmem/64};
        uint64_t stride_gmem[rank - 1] = {k_dim_gmem/2, 32};
        uint32_t dim_smem[rank] = {64, MN_SMEM_TD / CLUSTER_SIZE, K_SMEM_TD/64};
        uint32_t elem_stride[rank] = {1, 1, 1};
...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int PIPE_STAGES, int NUM_WARPS, int OUT_N_CHUNK, int CLUSTER_SIZE>
        ...
        if (warp_id == TMA_WARP) {
            ...
            if (elect_one_sync()) {
                auto tma_load_stage = [&](const int k_off, const int stage) {
                    ...
                    const int a_smem_stage_ptr = a_smem_ptr + stage * A_SMEM_TILESZ + cta_rank * (TD_SMEM_M / CLUSTER_SIZE) * (TD_SMEM_K / 2);
                    ...
                    tcgen05_3dtma_g2s_ab_multicast<1, CLUSTER_SIZE>(a_smem_stage_ptr, g_A_tmap, m_off + cta_rank * (TD_SMEM_M / CLUSTER_SIZE), k_off_coremat, mbar_addr_tma_stage, CacheHintSm100::EVICT_NORMAL);
...
void nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    if (N != cache_N || K != cache_K) {
        ...
        tma_3d_map_ab<M_TILE_SIZE, K_TILE_SIZE, CLUSTER_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled_fn, &tmap_a_temp, nullptr, M_TILE_SIZE, K);
        tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, 1, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled_fn, &tmap_b_temp, nullptr, N, K);
    ...
    // === KERNEL LAUNCH ===
    auto kernel_inst = nvfp4_group_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE_TYPE, PIPE_STAGES, NUM_WARPS, OUT_N_CHUNK, CLUSTER_SIZE>;
