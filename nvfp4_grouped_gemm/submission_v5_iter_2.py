...
__global__ void __cluster_dims__(1, 1, 1) nvfp4_group_gemm_kernel(const __grid_constant__ GroupDescs group_descs, const __grid_constant__ CUtensorMap tmap_a_temp, 
    ...
    */
    ...
    __shared__ alignas(8) int64_t mbars[PIPE_STAGES * 2 + 5];
    ...
    const int mbar_epi_meta_ready = mbar_addr_epi_done + 8;

    int tile_phase = 0, epi_done_phase = 0, mma_done_phase = 0, epi_meta_ready_phase = 0;

    if (warp_id == 0 && elect_one_sync()) {
        for (int i = 0; i < PIPE_STAGES * 2 + 5; i++) {
    ...
    while (true) {
        ...
        if (warp_id == TMA_WARP) {
            ...
            if (tile_idx < 0) {
                if (elect_one_sync()) {
                    group_smem = -1;
                    mbar_arrive(mbar_addr_tile_ready, 1);
                }
                break;
            }
        ...
        // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops
            ...
            if (elect_one_sync()) {
                ...
                mbar_arrive(mbar_epi_meta_ready, 1);

                // Publish shared state before signaling epilogue
                asm volatile("fence.proxy.async::generic.release.sync_restrict::shared::cta.cluster;" ::: "memory");
        ...
        // All warps aside from the two for TMA/MMA are dedicated to the epilogue
            ...
            mbar_wait(mbar_epi_meta_ready, epi_meta_ready_phase);
            epi_meta_ready_phase ^= 1;
