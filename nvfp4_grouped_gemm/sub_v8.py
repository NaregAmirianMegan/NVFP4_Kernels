    ...
    for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
        ...
        if (warp_id == TILE_WARP) {
            ...
            if (elect_one_sync()) {
                ...
                tmap_update<0>(local_SFA_tmap_addr, groups[group].sfa_addr, (groups[group].M * K / SF_BLOCK_SIZE) / 8);
        ...
        if (warp_id == TMA_WARP) {
            if (elect_one_sync()) {
                auto tma_load_stage = [&](const int k_off, const int stage) {
                    ...
                    */
                    const int sfa_off = ((k_off / 64) + (m_off / 128) * (K / 64)) * 512 / 8; // ISSUE: These could be just simple bit shifts, adjust if compiler doesn't
                    const int sfb_off = ((k_off / 64) + (n_off / 128) * (K / 64)) * 512 / 8;
                    tcgen05_1dtma_g2s_sf<CTA_GROUP>(sfa_smem_stage_ptr, g_SFA_tmap + tile_stage, sfa_off, mbar_addr_tma_stage, CacheHintSm100::EVICT_NORMAL);
                    tcgen05_1dtma_g2s_sf<CTA_GROUP>(sfb_smem_stage_ptr, g_SFB_tmap + tile_stage, sfb_off, mbar_addr_tma_stage, CacheHintSm100::EVICT_NORMAL);
...
std::vector<torch::Tensor> nvfp4_group_gemm(const std::vector<std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>>& abc_tensors, const std::vector<std::tuple<torch::Tensor, torch::Tensor>>& sf_tensors, const std::vector<std::tuple<int, int, int, int>>& prob_sizes, int N, int K, int G) {
    ...
    constexpr int CTA_GROUP = 1; // NOTE: This can only be 2 when transpose = true
