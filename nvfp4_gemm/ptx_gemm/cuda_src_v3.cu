...
/*
    Implement support for N=256 via TMEM indexing offsets
*/
    ...
    for (int k_off = 0; k_off < K; k_off += TD_SMEM_K) {
        ...
        if (warp_id == 0 && elect_one_sync()) {
            ...
            for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                ...
                if constexpr (TD_MMA_N == 256) {
                    uint64_t sfb_desc2 = make_smem_desc<0>(sfb_smem_ptr + SF_SMEM_TILESZ + (sub_k_iter * 512));
                    tcgen05_cp<1>(tmem_addr_sfb + 4 * sub_k_iter + 4, sfb_desc2);
                }
            }
