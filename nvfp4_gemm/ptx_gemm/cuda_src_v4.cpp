...
template<int TD_CTA_M, int TD_CTA_N,
         ...
         int TD_MMA_M, int TD_MMA_N, int TD_MMA_K, bool SWIZZLE>
__global__ void nvfp4_gemm_kernel(__half* __restrict__ c_ref, const int M, const int N, const int K, const __grid_constant__ CUtensorMap tmap_a,
    ...
    */
    ...
    constexpr int WIDTH_COREMAT = SWIZZLE ? 256 : 32; // ISSUE: This will change based on swizzle mode
    ...
    */
    ...
    __shared__ alignas(128) char sfb_smem[(1 + TD_MMA_N/256)*SF_SMEM_TILESZ];
    ...
    for (int k_off = 0; k_off < K; k_off += TD_SMEM_K) {
        ...
        if (warp_id == 0 && elect_one_sync()) {
            ...
            */
            ...
            if constexpr (TD_MMA_N == 256) {
                tcgen05_1dtma_g2s_sf(sfb_smem_ptr+SF_SMEM_TILESZ, sfb_g_ptr + (K / 64)*512, SF_SMEM_TILESZ, mbar_addr, CacheHintSm100::EVICT_NORMAL);
            }
        ...
        if (warp_id == 0 && elect_one_sync()) {
            ...
            for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                uint64_t sfa_desc = make_smem_desc<0, false>(sfa_smem_ptr + (sub_k_iter * 512)); // ISSUE: verify this should input 0 here
                uint64_t sfb_desc = make_smem_desc<0, false>(sfb_smem_ptr + (sub_k_iter * 512));
                tcgen05_cp<1>(tmem_addr_sfa + 4 * sub_k_iter, sfa_desc);
                tcgen05_cp<1>(tmem_addr_sfb + MAX<TD_MMA_N / 32, 4>() * sub_k_iter, sfb_desc);
                if constexpr (TD_MMA_N == 256) {
                    uint64_t sfb_desc2 = make_smem_desc<0, false>(sfb_smem_ptr + SF_SMEM_TILESZ + (sub_k_iter * 512));
            ...
            for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                ...
                if constexpr (SWIZZLE) {
                    a_desc = make_smem_desc<TD_MMA_M, SWIZZLE>(a_smem_ptr + sub_k_iter * 32);
                    b_desc = make_smem_desc<TD_MMA_N, SWIZZLE>(b_smem_ptr + sub_k_iter * 32);
                }
                else {
                    a_desc = make_smem_desc<TD_MMA_M, SWIZZLE>(a_smem_ptr + sub_k_iter * TD_MMA_K * TD_MMA_M / 2);
                    b_desc = make_smem_desc<TD_MMA_N, SWIZZLE>(b_smem_ptr + sub_k_iter * TD_MMA_K * TD_MMA_N / 2);
                }
...
torch::Tensor nvfp4_gemm(torch::Tensor a_ref, torch::Tensor b_ref, torch::Tensor sfa_ref, torch::Tensor sfb_ref, 
    ...
    tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled, &tmap_b, b_ref.data_ptr(), N, K);


    auto kernel_inst = nvfp4_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE>;
