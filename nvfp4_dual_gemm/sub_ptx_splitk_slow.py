...
__global__ void __cluster_dims__(CTA_GROUP, 1, 1) nvfp4_dual_gemm_kernel(
                                  float* __restrict__ workspace_1,    // [SPLIT_K][M][N] - FP32 partials for result 1
                                  float* __restrict__ workspace_2,    // [SPLIT_K][M][N] - FP32 partials for result 2
                                  const int M, const int N, const int K,
                                  const int k_per_split,              // K / SPLIT_K - each split processes this many K elements
    ...
    */
    ...
    // Split-K: compute K range for this split from blockIdx.z
    const int k_start = split_idx * k_per_split;
    const int k_end = k_start + k_per_split;

    // Workspace offset for this split's partial results
    const int workspace_split_offset = split_idx * M * N;
    ...
    */
    ...
    if (warp_id == TMA_WARP && elect_one_sync()) {
        auto tma_load_stage = [&](const int k_off) {
            // Use relative offset for stage calculation so each split starts at stage 0
        ...
        // Split-K: only process our assigned K range
    ...
    // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops, only one thread in one CTA of the pair executes
        ...
        for (int k_off = k_start; k_off < k_end; k_off += TD_SMEM_K) {
            ...
            for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                ...
                // Split-K: use (k_off - k_start) so first iteration of THIS split zeros accumulator
                int enable_input_d = (k_off - k_start) + sub_k_iter;
                tcgen05_mma_nvfp4<CTA_GROUP>(tmem_addr_result_1, a_desc, b1_desc, make_instr_desc<TD_MMA_M*CTA_GROUP, TD_MMA_N>(), sfa_tmem, sfb1_tmem, enable_input_d);
                tcgen05_mma_nvfp4<CTA_GROUP>(tmem_addr_result_2, a_desc, b2_desc, make_instr_desc<TD_MMA_M*CTA_GROUP, TD_MMA_N>(), sfa_tmem, sfb2_tmem, enable_input_d);
            }
            // signal MMA done
    ...
    // All warps aside from the two for TMA/MMA are dedicated to the epilogue
        ...
        for (int sub_m = 0; sub_m <  rows_per_warp / 16; sub_m++) {
            ...
            // Split-K: Write FP32 partials to workspace (no silu fusion yet!)
            // Reduction kernel will sum partials and apply epilogue fusion
            for (int i = 0; i < TD_MMA_N / 8; i++) {
                const int m_idx = m_off + warp_id * rows_per_warp + sub_m * 16 + lane_id / 4;
                const int n_idx = n_off + i * 8 + (lane_id % 4) * 2;
                const int out_idx = workspace_split_offset + m_idx * N + n_idx;

                // Write FP32 partials for result_1
                workspace_1[out_idx + 0] = results_1[i * 4 + 0];
                workspace_1[out_idx + 1] = results_1[i * 4 + 1];

                // Write FP32 partials for result_2
                workspace_2[out_idx + 0] = results_2[i * 4 + 0];
                workspace_2[out_idx + 1] = results_2[i * 4 + 1];

                // Handle the +8 row offset elements
                const int out_idx_row8 = workspace_split_offset + (m_idx + 8) * N + n_idx;
                workspace_1[out_idx_row8 + 0] = results_1[i * 4 + 2];
                workspace_1[out_idx_row8 + 1] = results_1[i * 4 + 3];
                workspace_2[out_idx_row8 + 0] = results_2[i * 4 + 2];
                workspace_2[out_idx_row8 + 1] = results_2[i * 4 + 3];
            }
        }
...
torch::Tensor nvfp4_dual_gemm(torch::Tensor a_ref, torch::Tensor b1_ref, torch::Tensor b2_ref,
                              torch::Tensor sfa_ref, torch::Tensor sfb1_ref, torch::Tensor sfb2_ref,
                              torch::Tensor c_ref, torch::Tensor workspace_1, torch::Tensor workspace_2,
...
def kernel(a_ref, b1_ref, b2_ref, sfa_ref, sfb1_ref, sfb2_ref, c_ref, workspace_1, workspace_2, M, N, K, SPLIT_K):
    return nvfp4_dual_gemm_module.nvfp4_dual_gemm(a_ref, b1_ref, b2_ref, sfa_ref, sfb1_ref, sfb2_ref, c_ref, workspace_1, workspace_2, M, N, K, SPLIT_K)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Choose SPLIT_K based on problem size
    # K must be divisible by (SPLIT_K * K_TILE_SIZE) where K_TILE_SIZE = 256
    SPLIT_K = 4

    # Allocate FP32 workspace for partial results: [SPLIT_K, M, N]
    workspace_1 = torch.empty((SPLIT_K, M, N), dtype=torch.float32, device=a_ref.device)
    workspace_2 = torch.empty((SPLIT_K, M, N), dtype=torch.float32, device=a_ref.device)

    return kernel(a_ref, b1_ref, b2_ref, sfa_ref_permuted, sfb1_ref_permuted, sfb2_ref_permuted, c_ref, workspace_1, workspace_2, M, N, K, SPLIT_K)
