...
__global__ void nvfp4_dual_gemm_kernel(__half* __restrict__ c_ref, const int M, const int N, const int K, const __grid_constant__ CUtensorMap tmap_a,
    ...
    */
    ...
    __shared__ float outputs[OUT_SZ];
    ...
    */
    ...
    const int split_k_id = cta_rank / CTA_GROUP;
    const int cta_id_m = cta_rank % CTA_GROUP;
    //int split_k_id;
    //asm volatile("mov.u32 %0, %%cluster_ctaid.z;" : "=r"(split_k_id));

    const int k_start = split_k_id * (K / SPLIT_K);
    const int k_end = k_start + (K / SPLIT_K);
    ...
    if (warp_id == TMA_WARP && elect_one_sync()) {
        auto tma_load_stage = [&](const int k_off) {
            ...
            // DEBUG
            /*
            if (m_off*(K/2) + k_off_coremat*128 + TD_SMEM_M*(TD_SMEM_K/2) >= M*(K/2)) {
                printf("(%d, %d) ", blockIdx.x, blockIdx.y);
            }
            if ((n_off + cta_id_m * (TD_MMA_N / 2))*(K/2) + k_off_coremat*128 + TD_SMEM_N*(TD_SMEM_K/2) >= N*(K/2)) {
                printf("(%d, %d) ", blockIdx.x, blockIdx.y);
            }
            */

            tcgen05_3dtma_g2s_ab<CTA_GROUP>(a_smem_stage_ptr, &tmap_a, m_off, k_off_coremat, mbar_addr_tma_stage, CacheHintSm100::EVICT_LAST);
            tcgen05_3dtma_g2s_ab<CTA_GROUP>(b1_smem_stage_ptr, &tmap_b1, n_off + cta_id_m * (TD_MMA_N / 2), k_off_coremat, mbar_addr_tma_stage, CacheHintSm100::EVICT_FIRST);
            tcgen05_3dtma_g2s_ab<CTA_GROUP>(b2_smem_stage_ptr, &tmap_b2, n_off + cta_id_m * (TD_MMA_N / 2), k_off_coremat, mbar_addr_tma_stage, CacheHintSm100::EVICT_FIRST);
            ...
            */
            ...
            // DEBUG
            /*
            if (k_off_sf*256 + m_block_off*(512ULL * (K / 64)) + 512ULL * (TD_SMEM_K / 64) >= M*(K/16)) {
                printf("ASF_OVERUN: (%d, %d) ", blockIdx.x, blockIdx.y);
            }
            if (k_off_sf*256 + n_block_off*(512ULL * (K / 64)) + 512ULL * (TD_SMEM_K / 64) >= N*(K/16)) {
                printf("BSF_OVERUN: (%d, %d) ", blockIdx.x, blockIdx.y);
            }
            */
    ...
    // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops, only one thread in one CTA of the pair executes
    else if (cta_id_m == 0 && warp_id == MMA_WARP && elect_one_sync()) {
        for (int k_off = k_start; k_off < k_end; k_off += TD_SMEM_K) {
            const int stage = ((k_off-k_start) / TD_SMEM_K) % PIPE_STAGES;
            ...
            for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                ...
                tcgen05_mma_nvfp4<CTA_GROUP>(tmem_addr_result_1, a_desc, b1_desc, make_instr_desc<TD_MMA_M*CTA_GROUP, TD_MMA_N>(), sfa_tmem, sfb1_tmem, (k_off - k_start) + sub_k_iter); // Inputting k_off like this will set enable-input-d so only on the first mma we 0 out the result space in TMEM
                tcgen05_mma_nvfp4<CTA_GROUP>(tmem_addr_result_2, a_desc, b2_desc, make_instr_desc<TD_MMA_M*CTA_GROUP, TD_MMA_N>(), sfa_tmem, sfb2_tmem, (k_off - k_start) + sub_k_iter);
            }
            // signal MMA done
        ...
        if (warp_id == 0) { tcgen05_dealloc_tmem<CTA_GROUP>(tmem_addr_result_1, TD_MMA_N * 4); } // DEBUG

        return;

        // Write values for the section of results this CTA is responsible for
        // For SPLIT_K == 2
            // We need to store a (TD_MMA_M/2, TD_MMA_K) = (64, TD_MM_K) segment of each TMEM result to SMEM
            // Each warp loads 16 rows, so no loop is required
        // For SPLIT_K == 4
            // Only a (32, TD_MM_K) segment of each TMEM result

        float results_1[TD_MMA_N/2];
        float results_2[TD_MMA_N/2];
        const int row_offset = (cta_id_m * TD_MMA_M + split_k_id * (TD_MMA_M / SPLIT_K) + warp_id * 16) << 16;
        tcgen05_ld<16, 256, TD_MMA_N/8>(results_1, tmem_addr_result_1 + row_offset);
        tcgen05_ld<16, 256, TD_MMA_N/8>(results_2, tmem_addr_result_2 + row_offset);
        asm volatile("tcgen05.wait::ld.sync.aligned;");

        for (int i = 0; i < TD_MMA_N / 8; i++) {
            const int m_offset = warp_id * 16 + lane_id / 4;
            const int n_offset = i * 8 + (lane_id % 4) * 2;
            outputs[m_offset*TD_MMA_N + n_offset] = results_1[i*4]; // ISSUE: Convert to uint64_t so these can be coalesced
            outputs[m_offset*TD_MMA_N + n_offset + 1] = results_1[i*4 + 1];
            outputs[(m_offset+8)*TD_MMA_N + n_offset] = results_1[i*4 + 2];
            outputs[(m_offset+8)*TD_MMA_N + n_offset + 1] = results_1[i*4 + 3];

            outputs[(OUT_SZ / 2) + m_offset*TD_MMA_N + n_offset] = results_2[i*4];
            outputs[(OUT_SZ / 2) + m_offset*TD_MMA_N + n_offset + 1] = results_2[i*4 + 1];
            outputs[(OUT_SZ / 2) + (m_offset+8)*TD_MMA_N + n_offset] = results_2[i*4 + 2];
            outputs[(OUT_SZ / 2) + (m_offset+8)*TD_MMA_N + n_offset + 1] = results_2[i*4 + 3];
        }
    }
    return; // DEBUG
    // Ensure all CTA results SMEMs are initialized with local CTA results
    __syncthreads(); // ISSUE: Do we need this?
    cluster.sync();

    if (warp_id < NUM_WARPS - 2) {
        // Atomically add this CTA's results to other CTAs in the cluster
        float results_1[TD_MMA_N/2];
        float results_2[TD_MMA_N/2];
        const int row_offset = (cta_id_m * TD_MMA_M + (1 - split_k_id) * (TD_MMA_M / SPLIT_K) + warp_id * 16) << 16;
        tcgen05_ld<16, 256, TD_MMA_N/8>(results_1, tmem_addr_result_1 + row_offset);
        tcgen05_ld<16, 256, TD_MMA_N/8>(results_2, tmem_addr_result_2 + row_offset);
        asm volatile("tcgen05.wait::ld.sync.aligned;");

        float *outputs_other = cluster.map_shared_rank(outputs, cta_id_m + SPLIT_K * (1-split_k_id));


        for (int i = 0; i < TD_MMA_N / 8; i++) {
            const int m_offset = warp_id * 16 + lane_id / 4;
            const int n_offset = i * 8 + (lane_id % 4) * 2;
            outputs_other[m_offset*TD_MMA_N + n_offset] = results_1[i*4]; // ISSUE: Convert to uint64_t so these can be coalesced
            outputs_other[m_offset*TD_MMA_N + n_offset + 1] = results_1[i*4 + 1];
            outputs_other[(m_offset+8)*TD_MMA_N + n_offset] = results_1[i*4 + 2];
            outputs_other[(m_offset+8)*TD_MMA_N + n_offset + 1] = results_1[i*4 + 3];

            outputs_other[(OUT_SZ / 2) + m_offset*TD_MMA_N + n_offset] = results_2[i*4];
            outputs_other[(OUT_SZ / 2) + m_offset*TD_MMA_N + n_offset + 1] = results_2[i*4 + 1];
            outputs_other[(OUT_SZ / 2) + (m_offset+8)*TD_MMA_N + n_offset] = results_2[i*4 + 2];
            outputs_other[(OUT_SZ / 2) + (m_offset+8)*TD_MMA_N + n_offset + 1] = results_2[i*4 + 3];
        }
    }

    // Ensure all CTAs have results from other CTAs in the cluster (i.e. other k-splits)
    __syncthreads(); // ISSUE: Do we need this?
    cluster.sync();
    
    if (warp_id < NUM_WARPS - 2) {
        // Merge local SMEM output buffers via silu(x)*y and store to GMEM
        // We have (TD_MMA_M / SPLIT_K, TD_MMA_N) segments to merge using 4 warps
        // Each warp handles (TD_MMA_M / SPLIT_K) / 4 rows
        for (int i = 0; i < (TD_MMA_M / SPLIT_K) / 4; ++i) {
            int row = warp_id * ((TD_MMA_M / SPLIT_K) / 4) + i;
            int col = lane_id;
            float out1 = outputs[row * TD_MMA_N + col];
            float out2 = outputs[(OUT_SZ / 2) + row * TD_MMA_N + col];
            float result0 = silu(out1) * out2;

            col = 32 + lane_id;
            out1 = outputs[row * TD_MMA_N + col];
            out2 = outputs[(OUT_SZ / 2) + row * TD_MMA_N + col];
            float result1 = silu(out1) * out2;

            int m_offset = m_off + split_k_id * (TD_MMA_M / SPLIT_K) + row;
            c_ref[m_offset*N + n_off + lane_id] = __float2half_rn(result0);
            c_ref[m_offset*N + n_off + lane_id + 32] = __float2half_rn(result1);
        }
...
torch::Tensor nvfp4_dual_gemm(torch::Tensor a_ref, torch::Tensor b1_ref, torch::Tensor b2_ref, torch::Tensor sfa_ref, torch::Tensor sfb1_ref, torch::Tensor sfb2_ref, torch::Tensor c_ref, int M, int N, int K) { 
    ...
    auto kernel_inst = nvfp4_dual_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE, PIPE_STAGES, NUM_WARPS, CTA_GROUP, SPLIT_K>;
    ...
    config.gridDim.y = SPLIT_K * N/N_TILE_SIZE;
    config.gridDim.z = 1;
    ...
    attribute[0].val.clusterDim.y = SPLIT_K;
    ...
    //dim3 grid_dim(M/M_TILE_SIZE, N/N_TILE_SIZE, SPLIT_K); // If CTA_GROUP == 2, adjacent CTAs along the grid X dimension are paired into CTA clusters
