#!POPCORN leaderboard final_nvfp4_dual_gemm
import torch
...
__global__ void nvfp4_dual_gemm_kernel(__half* __restrict__ c_ref, const int M, const int N, const int K, const __grid_constant__ CUtensorMap tmap_a,
                                  const __grid_constant__ CUtensorMap tmap_b1, const __grid_constant__ CUtensorMap tmap_b2, const uint8_t* sfa_gmem_base, 
                                  const uint8_t* sfb1_gmem_base, const uint8_t* sfb2_gmem_base) {
    /*
    ...
    */
    ...
    constexpr int SFA_SMEM_TILESZ = 128 * (TD_SMEM_K / SF_BLOCK_SIZE);
    constexpr int SFB1_SMEM_TILESZ = 128 * (TD_SMEM_K / SF_BLOCK_SIZE);
    constexpr int SFB2_SMEM_TILESZ = 128 * (TD_SMEM_K / SF_BLOCK_SIZE);
    constexpr int A_SMEM_TILESZ = TD_SMEM_M * (TD_SMEM_K / 2);
    constexpr int B1_SMEM_TILESZ = TD_SMEM_N * (TD_SMEM_K / 2);
    constexpr int B2_SMEM_TILESZ = TD_SMEM_N * (TD_SMEM_K / 2);
    ...
    */
    ...
    if (warp_id == 0 && elect_one_sync()) {
        ...
        tcgen05_alloc_tmem<1>(&tmem_addr_ptr[0], TD_MMA_N*4);
    }
    ...
    // MMA thread loops over SMEM tile stages, loads TMEM and computes MMA ops
        ...
        for (int k_off = 0; k_off < K; k_off += TD_SMEM_K) {
            ...
            for (int sub_k_iter = 0; sub_k_iter < TD_SMEM_K / TD_MMA_K; sub_k_iter++) {
                ...
                tcgen05_mma_nvfp4<1>(tmem_addr_result_1, a_desc, b1_desc, make_instr_desc<TD_MMA_M, TD_MMA_N>(), sfa_tmem, sfb1_tmem, k_off + sub_k_iter); // Inputting k_off like this will set enable-input-d so only on the first mma we 0 out the result space in TMEM
    ...
    // All warps aside from the two for TMA/MMA are dedicated to the epilogue
        ...
        if (warp_id == 0) { tcgen05_dealloc_tmem<1>(tmem_addr_result_1, TD_MMA_N * 4); }
    } 
}
...
torch::Tensor nvfp4_dual_gemm(torch::Tensor a_ref, torch::Tensor b1_ref, torch::Tensor b2_ref, torch::Tensor sfa_ref, torch::Tensor sfb1_ref, torch::Tensor sfb2_ref, torch::Tensor c_ref, int M, int N, int K) { 
    ...
    CUtensorMap tmap_a, tmap_b1, tmap_b2;
    ...
    tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled, &tmap_b1, b1_ref.data_ptr(), N, K);
    tma_3d_map_ab<N_TILE_SIZE, K_TILE_SIZE, SWIZZLE_TYPE>::init(cuTensorMapEncodeTiled, &tmap_b2, b2_ref.data_ptr(), N, K);


    auto kernel_inst = nvfp4_dual_gemm_kernel<M_TILE_SIZE, N_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_TILE_SIZE, M_TILE_SIZE, N_TILE_SIZE, K_MMA_SIZE, SWIZZLE, PIPE_STAGES, NUM_WARPS>;
    ...
    kernel_inst<<<grid_dim, threads>>>(reinterpret_cast<__half*>(c_ref.data_ptr()), M, N, K, tmap_a, tmap_b1, tmap_b2, reinterpret_cast<uint8_t*>(sfa_ref.data_ptr()), reinterpret_cast<uint8_t*>(sfb1_ref.data_ptr()), reinterpret_cast<uint8_t*>(sfb2_ref.data_ptr()));
