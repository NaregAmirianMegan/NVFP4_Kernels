...
#define NUM_STAGES 5  // Number of pipeline stages (buffered k-blocks)
...
__global__ void batched_nvfp4_gemv_kernel(const __nv_fp4x2_storage_t* __restrict__ a_ref, const __nv_fp4x2_storage_t* __restrict__ b_ref, 
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        int read_stage = kb % NUM_STAGES;
        
        // Launch async load for block at kb + NUM_STAGES - 1
        int prefetch_kb = kb + NUM_STAGES - 1;
        if (prefetch_kb < k_blocks) {
            int write_stage = prefetch_kb % NUM_STAGES;
            
            if (l_wid == 0) {
                cuda::memcpy_async(&b_smem[write_stage][l_wtid],
                                  &reinterpret_cast<uint64_t const*>(b_ref)[(b_base + prefetch_kb*(K_BLOCK_SIZE/2))/8 + l_wtid],
                                  sizeof(uint64_t), pipe);
                cuda::memcpy_async(&sfb_smem[write_stage][l_wtid],
                                  &sfb_ref[sfb_base + prefetch_kb*32 + l_wtid],
                                  sizeof(__nv_fp8_storage_t), pipe);
            }
            for (int result = 0; result < RESULTS_PER_WARP; result++) {
                cuda::memcpy_async(&a_smem[write_stage][l_wid*RESULTS_PER_WARP + result][l_wtid],
                                  &reinterpret_cast<uint64_t const*>(a_ref)[(a_base + l_wid*RESULTS_PER_WARP*k_elems + result*k_elems + prefetch_kb*(K_BLOCK_SIZE/2))/8 + l_wtid],
                                  sizeof(uint64_t), pipe);
                cuda::memcpy_async(&sfa_smem[write_stage][l_wid*RESULTS_PER_WARP + result][l_wtid],
                                  &sfa_ref[sfa_base + l_wid*RESULTS_PER_WARP*k_scalars + result*k_scalars + prefetch_kb*32 + l_wtid],
                                  sizeof(__nv_fp8_storage_t), pipe);
            }
            pipe.producer_commit();
        }

        // Wait for the current block's async copies to complete
        // We need to wait for the copy that was issued NUM_STAGES-1 iterations ago
        pipe.consumer_wait();
        __syncthreads();

        // Compute on current k-block from read_stage
        __nv_fp4x2_storage_t const* b_vals = reinterpret_cast<__nv_fp4x2_storage_t const*>(&b_smem[read_stage][l_wtid]);

        __nv_fp8_storage_t sfb = sfb_smem[read_stage][l_wtid];
        ...
        #pragma unroll
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            __nv_fp4x2_storage_t const* a_vals = reinterpret_cast<__nv_fp4x2_storage_t const*>(&a_smem[read_stage][l_wid*RESULTS_PER_WARP + result][l_wtid]);

            __nv_fp8_storage_t sfa = sfa_smem[read_stage][l_wid*RESULTS_PER_WARP + result][l_wtid];
