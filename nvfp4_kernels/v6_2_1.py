...
"""
This is a modification on top of v6_2, where we only perform the B,SFB conversions on one warp per CTA
instead of on every thread, in order to reduce the number of logical operations needed for type conversion
and address resolution. An artifact of this is now that each thread sources data in the inner compute loop
from shared memory, we need to sync threads after the compute loop in order to prevent threads from
overwriting SMEM data for the next k-block that other threads are currently using for this k-block computation.
