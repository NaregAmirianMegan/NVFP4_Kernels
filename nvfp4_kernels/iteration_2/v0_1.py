...
"""
Have each thread track it's own result sums, then do warp shuffle reduction at the 
end outside of the k_block loop. This should reduce the amount of time spent on warp
shuffling by a factor of k_dim/d_block_size (so larger k will benefit more from this).
The only question is will this increase our register usage and thus decrease occupancy?

As suspected this doesn't increase the register per thread count, and so this gives us 
...
__global__ void batched_nvfp4_gemv_kernel(const __nv_fp4x2_storage_t* __restrict__ a_ref, const __nv_fp4x2_storage_t* __restrict__ b_ref, 
    ...
    for (int result = 0; result < RESULTS_PER_WARP; result++) {
        for (int offset = 16; offset > 0; offset >>= 1) {
            results[result] += __shfl_down_sync(0xffffffff, result[results], offset);
        }
    }
