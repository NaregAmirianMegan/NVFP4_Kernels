...
"""
Moved b_vals_cvt to SMEM to decrease reg pressure and parellelize the conversions. But this
increases latency in the inner loop because you're loading the converted values from SMEM not
directly accessing local regs, and it increases reg pressure since we need additional regs for
indexing, and we still need the regs to bring the converted values in.
"""
...
__global__ void batched_nvfp4_gemv_kernel(const __nv_fp4x2_storage_t* __restrict__ a_ref, const __nv_fp4x2_storage_t* __restrict__ b_ref, 
    ...
    __shared__ uint32_t b_vals_cvt[(K_BLOCK_SIZE/8)*4];
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        // Each thread loads 8 NVFP4 vals from b_smem, converts them to 8 FP16 vals and stores them to b_vals_cvt
        // Each k-block is 1024 elements, divided by 8 elements converted per thread = 128 threads needed
        if (threadIdx.x < K_BLOCK_SIZE/8) {
            asm volatile(
                "{\\n"
                ...
                "}\\n"
                    : "=r"(b_vals_cvt[threadIdx.x*4]), "=r"(b_vals_cvt[threadIdx.x*4+1]), "=r"(b_vals_cvt[threadIdx.x*4+2]), "=r"(b_vals_cvt[threadIdx.x*4+3])
                    : "r"(b_smem[threadIdx.x])
                    : "memory"
                );
        }

        // We have to sync threads again after conversion to ensure all warps get the proper converted vals from SMEM
        __syncthreads();
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            asm volatile(
                ...
                : "h"(sfa_smem[(warp_result_offset + result)*(K_BLOCK_SIZE/32) + l_wtid]), "r"(sfb_fp16x2),        // 1, 2
                  ...
                  "r"(b_vals_cvt[l_wtid*16]), "r"(b_vals_cvt[l_wtid*16+1]),   // 7, 8
                  "r"(b_vals_cvt[l_wtid*16+2]), "r"(b_vals_cvt[l_wtid*16+3]),    // 9, 10
                  "r"(b_vals_cvt[l_wtid*16+4]), "r"(b_vals_cvt[l_wtid*16+5]),   // 11, 12
                  "r"(b_vals_cvt[l_wtid*16+6]), "r"(b_vals_cvt[l_wtid*16+7]),    // 13, 14
                  "r"(b_vals_cvt[l_wtid*16+8]), "r"(b_vals_cvt[l_wtid*16+9]),   // 15, 16
                  "r"(b_vals_cvt[l_wtid*16+10]), "r"(b_vals_cvt[l_wtid*16+11]),    // 17, 18
                  "r"(b_vals_cvt[l_wtid*16+12]), "r"(b_vals_cvt[l_wtid*16+13]),   // 19, 20
                  "r"(b_vals_cvt[l_wtid*16+14]), "r"(b_vals_cvt[l_wtid*16+15])    // 21, 22
                : "memory"
            );
