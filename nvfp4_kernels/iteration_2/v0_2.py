...
"""
Our stores aren't being coalesced because thread0 in each warp
holds RESULTS_PER_WARP results in registers that then get converted
and stored in a loop to GMEM, resulting in storing 2B per memory transaction
to GMEM which isn't great.

In order to coalesce the writes at least within each warp we can first convert
all the result values from float -> half, then perform a vectorized store to GMEM
resulting in 1 RESULTS_PER_WARP*2B transaction instead of RESULTS_PER_WARP individual
2B transactions.
"""
