...
"""
In v1 we tried implementing double-buffer pipelining of loading memory and computing, but
this only provided a minor speed-up. This is because when transferring non-contiguous 2D segments
like a chunk of a or sfa, we still need to initiate async transfers in warp coalesced chunks,
which means the data loading itself still takes a while to dispatch. This means we are likely
overlapping memory ops with other memory ops until our 2-stage pipe gets backed up (which would
happen quickly) and our performance regresses back to as if we were sequentially loading and
processing data like in v0_2.py

In order to quickly dispatch our data transfers we can utilize bulk tensor transfers with the TMA
(Tensor Memory Accelerator) engine. This should significantly decrease async copy dispatch time and
truly start overlapping compute with memory transfer, leading to a better speedup.
"""
