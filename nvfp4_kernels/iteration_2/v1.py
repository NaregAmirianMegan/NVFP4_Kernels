...
"""
We are seeing lots of long-scoreboard stalls and sync stalls. Both of these are likely
due to our data-loading section being serialized with computation. If we use asyncrhonous
transfers and overlap data movement with compute we should increase memory bandwidth and
computational throughput, resulting in faster runtimes.

In this version we try a simple double buffer. So we load into one set of SMEM buffers
while computing on the other set, swapping back and forth using async memory copies.
"""
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            asm volatile(
                ...
                : "h"(sfa_smem[read_buf*(NUM_WARPS_PER_CTA*RESULTS_PER_WARP*K_BLOCK_SIZE/32) + (warp_result_offset + result)*(K_BLOCK_SIZE/32) + l_wtid]), "r"(sfb_fp16x2),        // 1, 2
                  "r"(a_smem[read_buf*(NUM_WARPS_PER_CTA*RESULTS_PER_WARP*K_BLOCK_SIZE/8) + (warp_result_offset + result)*(K_BLOCK_SIZE/8) + l_wtid*4]), "r"(a_smem[read_buf*(NUM_WARPS_PER_CTA*RESULTS_PER_WARP*K_BLOCK_SIZE/8) + (warp_result_offset + result)*(K_BLOCK_SIZE/8) + l_wtid*4 + 1]),   // 3, 4
                  "r"(a_smem[read_buf*(NUM_WARPS_PER_CTA*RESULTS_PER_WARP*K_BLOCK_SIZE/8) + (warp_result_offset + result)*(K_BLOCK_SIZE/8) + l_wtid*4 + 2]), "r"(a_smem[read_buf*(NUM_WARPS_PER_CTA*RESULTS_PER_WARP*K_BLOCK_SIZE/8) + (warp_result_offset + result)*(K_BLOCK_SIZE/8) + l_wtid*4 + 3]),   // 5, 6
