...
"""
We are trying to further improve our register usage and efficiency of the inner computation loop.

Here we tried to reduce register pressure by storing results in SMEM rather than register buffers, this
resulted in a slight slowdown however.
"""
...
__global__ void batched_nvfp4_gemv_kernel(const __nv_fp4x2_storage_t* __restrict__ a_ref, const __nv_fp4x2_storage_t* __restrict__ b_ref, 
    ...
    __shared__ float results[NUM_WARPS_PER_CTA][RESULTS_PER_WARP];

    // All threads cooperatively initialize
    int total_results = NUM_WARPS_PER_CTA * RESULTS_PER_WARP;
    if (threadIdx.x < total_results) {
        reinterpret_cast<float*>(results)[threadIdx.x] = 0.0f;
    }
    __syncthreads(); // Ensure initialization completes before use
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            if (l_wtid == 0) {
                results[l_wid][result] += sum;
            }
        }
    }

    // Write all results back to GMEM 
    if (l_wtid < RESULTS_PER_WARP) {
        c_ref[batch*m + cta_row_start + RESULTS_PER_WARP*l_wid + l_wtid] = __float2half(results[l_wid][l_wtid]);
    }
}
