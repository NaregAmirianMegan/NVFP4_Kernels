...
"""
Here we make some small modifications to reduce register usage (to increase occupancy and avoid a costly tail-case wave).
We also removed the unnecessary __syncwarps() call in the results loop because __shfl_down_sync already syncs threads
in a warp.

There was an interesting lesson in the process of creating this version. When we got rid of the pragma unroll it exposed
the use of the unnecessary __syncwarps() call through a correctness error. (will need to look more at the assembly
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            for (int i = 0; i < 8; i+=2) {
                ...
                sub_total = __hfma2(a0_2x_fp16, b_vals_cvt[i], sub_total);
                __half2 a1_2x_fp16 = *reinterpret_cast<__half2*>(&a1_2x_raw_fp16);
                sub_total = __hfma2(a1_2x_fp16, b_vals_cvt[i+1], sub_total);
            }
