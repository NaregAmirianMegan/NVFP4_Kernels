...
__global__ void batched_nvfp4_gemv_kernel(const __nv_fp4x2_storage_t* __restrict__ a_ref, const __nv_fp4x2_storage_t* __restrict__ b_ref, 
    ...
    int write_buf = 0;
    if (l_wid == 0) {
        b_smem[write_buf][l_wtid] = reinterpret_cast<uint64_t const*>(b_ref)[(b_base)/8 + l_wtid];
        sfb_smem[write_buf][l_wtid] = sfb_ref[sfb_base + l_wtid];
    }
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        // Sync before next iteration (ensures write_buf loads complete before reading)
        __syncthreads();
    }
