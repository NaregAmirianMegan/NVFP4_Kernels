...
#define RESULTS_PER_WARP 1
#define NUM_WARPS_PER_CTA 8
#define N_DIM_PADDING 128
#define K_BLOCK_SIZE 4096 // 32 threads per warp * 128 NVFP4 elems per thread = 4096 elems per warp (warp handles one k_block)

#define CEIL_DIV(NUM, DIV) ((NUM + DIV - 1) / DIV)

__device__ inline uint16_t compute_32_elems(uint16_t sfa_fp8x2, uint32_t sfb_fp16x2, uint64_t a_smem_0, uint64_t a_smem_1,
                                            uint64_t b_smem_0, uint64_t b_smem_1) {
    ...
    asm volatile(
        ...
        "mov.b32 {a_byte0, a_byte1, a_byte2, a_byte3}, a_lo;\\n"
        "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, b_lo;\\n"
        ...
        "mov.b32 {a_byte0, a_byte1, a_byte2, a_byte3}, a_hi;\\n"
        "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, b_hi;\\n"
        ...
        "}\\n"
        : "=h"(result_fp16)                                     // 0
        : "h"(sfa_fp8x2), "r"(sfb_fp16x2),        // 1, 2
          "l"(a_smem_0), "l"(a_smem_1),   // 3, 4
          "l"(b_smem_0), "l"(b_smem_1)    // 5, 6
        : "memory"
    );
...
__global__ void batched_nvfp4_gemv_kernel(int m, int k, int l, const __grid_constant__ CUtensorMap tensor_map_a,
    ...
    __shared__ alignas(128) uint64_t a_smem[2][NUM_WARPS_PER_CTA*RESULTS_PER_WARP][K_BLOCK_SIZE/16];
    __shared__ alignas(128) uint16_t sfa_smem[2][NUM_WARPS_PER_CTA*RESULTS_PER_WARP][K_BLOCK_SIZE/32];
    __shared__ alignas(128) uint64_t b_smem[2][K_BLOCK_SIZE/16];
   ...
   __shared__ alignas(128) uint16_t c_smem[NUM_WARPS_PER_CTA*RESULTS_PER_WARP];
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        if (kb+1 < k_blocks) {
            ...
            if (threadIdx.x == 0) {
                // Load a block
                cde::cp_async_bulk_tensor_2d_global_to_shared(&a_smem[load_stage], &tensor_map_a, (kb+1)*(K_BLOCK_SIZE/16), cta_row, bar[load_stage]);
                // Load sfa block
                ...
                // Load b block
                cde::cp_async_bulk_tensor_2d_global_to_shared(&b_smem[load_stage], &tensor_map_b, (kb+1)*(K_BLOCK_SIZE/16), batch, bar[load_stage]);
                // Load sfb block
        ...
        uint32_t sfb_fp16x2_0, sfb_fp16x2_1, sfb_fp16x2_2, sfb_fp16x2_3;
        asm volatile(
            "{\\n"
            "cvt.rn.f16x2.e4m3x2 %0, %4;\\n"
            "cvt.rn.f16x2.e4m3x2 %1, %5;\\n"
            "cvt.rn.f16x2.e4m3x2 %2, %6;\\n"
            "cvt.rn.f16x2.e4m3x2 %3, %7;\\n"
            "}\\n"
                : "=r"(sfb_fp16x2_0), "=r"(sfb_fp16x2_1),
                  "=r"(sfb_fp16x2_2), "=r"(sfb_fp16x2_3)
                : "h"(sfb_smem[comp_stage][l_wtid*4]), "h"(sfb_smem[comp_stage][l_wtid*4 + 1]),
                  "h"(sfb_smem[comp_stage][l_wtid*4 + 2]), "h"(sfb_smem[comp_stage][l_wtid*4 + 3])
                : "memory"
            );
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            uint16_t result_fp16 = compute_32_elems(sfa_smem[comp_stage][warp_result_offset + result][l_wtid*4], sfb_fp16x2_0, a_smem[comp_stage][warp_result_offset + result][l_wtid*8], 
                                            a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 1], b_smem[comp_stage][l_wtid*8], b_smem[comp_stage][l_wtid*8 + 1]);

            float sum = __half2float(__ushort_as_half(result_fp16));

            if (kb*K_BLOCK_SIZE + l_wtid*128 + 32 < k) {

                result_fp16 = compute_32_elems(sfa_smem[comp_stage][warp_result_offset + result][l_wtid*4+1], sfb_fp16x2_1, a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 2], 
                                        a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 3], b_smem[comp_stage][l_wtid*8 + 2], b_smem[comp_stage][l_wtid*8 + 3]);

                sum += __half2float(__ushort_as_half(result_fp16));
            }

            if (kb*K_BLOCK_SIZE + l_wtid*128 + 64 < k) {

                result_fp16 = compute_32_elems(sfa_smem[comp_stage][warp_result_offset + result][l_wtid*4+2], sfb_fp16x2_2, a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 4], 
                                        a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 5], b_smem[comp_stage][l_wtid*8 + 4], b_smem[comp_stage][l_wtid*8 + 5]);

                sum += __half2float(__ushort_as_half(result_fp16));
            }

            if (kb*K_BLOCK_SIZE + l_wtid*128 + 96 < k) {

                result_fp16 = compute_32_elems(sfa_smem[comp_stage][warp_result_offset + result][l_wtid*4+3], sfb_fp16x2_3, a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 6], 
                                        a_smem[comp_stage][warp_result_offset + result][l_wtid*8 + 7], b_smem[comp_stage][l_wtid*8 + 6], b_smem[comp_stage][l_wtid*8 + 7]);
...
torch::Tensor batched_nvfp4_gemv(torch::Tensor a_ref, torch::Tensor b_ref, torch::Tensor sfa_ref, torch::Tensor sfb_ref, 
    ...
    const uint64_t GMEM_WIDTH_A = k/16;
    const uint64_t GMEM_HEIGHT_A = m*l;
    const uint32_t SMEM_WIDTH_A = K_BLOCK_SIZE/16;
    ...
    uint64_t stride_a[rank_a - 1] = {GMEM_WIDTH_A * sizeof(uint64_t)};
    ...
    const uint64_t GMEM_WIDTH_B = k/16;
    const uint64_t GMEM_HEIGHT_B = l;
    const uint32_t SMEM_WIDTH_B = K_BLOCK_SIZE/16;
    ...
    // The stride is the number of bytes to traverse from the first element of one row to the next.
    // It must be a multiple of 16.
    uint64_t stride_b[rank_b - 1] = {GMEM_WIDTH_B * sizeof(uint64_t) * N_DIM_PADDING};
    // The box_size is the size of the shared memory buffer that is used as the
    // destination of a TMA transfer.
