...
#define RESULTS_PER_CTA NUM_WARPS_PER_CTA * RESULTS_PER_WARP
#define N_DIM_PADDING 128
#define K_BLOCK_SIZE 256 // 32 threads per warp * 32 NVFP4 elems per thread = 1024 elems per warp (warp handles one k_block)
...
__global__ void batched_nvfp4_gemv_kernel(int m, int k, int l, const __grid_constant__ CUtensorMap tensor_map_a,
    ...
    __shared__ alignas(128) uint32_t a_smem[2][RESULTS_PER_CTA][K_BLOCK_SIZE/8];
    __shared__ alignas(128) uint8_t sfa_smem[2][RESULTS_PER_CTA][K_BLOCK_SIZE/16];
    __shared__ alignas(128) uint32_t b_smem[2][K_BLOCK_SIZE/8];
    __shared__ alignas(128) uint8_t sfb_smem[2][ 128 * (((K_BLOCK_SIZE/16) + 128 - 1) / 128) ];

   __shared__ alignas(128) uint16_t c_smem[RESULTS_PER_CTA];

    const int total_smem_size = sizeof(a_smem[0]) + sizeof(sfa_smem[0]) + sizeof(b_smem[0]) + sizeof(sfb_smem[0]);
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        asm volatile(
            ...
            "}\\n"
                : "=r"(sfb_fp16x2)
                : "r"((uint32_t)sfb_smem[comp_stage][l_wtid/2])
                : "memory"
            );
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            asm volatile(
                ...
                "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, %4;\\n"
                ...
                "mul.rn.f16x2 accum_0, accum_0, sf_f16x2;\\n"
                ...
                "}\\n"
                : "=h"(result_fp16)                                     // 0
                : "r"((uint32_t)sfa_smem[comp_stage][warp_result_offset + result][l_wtid/2]), "r"(sfb_fp16x2),        // 1, 2
                  "r"(a_smem[comp_stage][warp_result_offset + result][l_wtid]),
                  "r"(b_smem[comp_stage][l_wtid])
                : "memory"
            );
    ...
    if (l_wtid == 0) {
        ...
        //reinterpret_cast<float4*>(c_smem)[(RESULTS_PER_WARP*l_wid)/8 + 1] = reinterpret_cast<float4*>(results_half)[1];
    }
...
torch::Tensor batched_nvfp4_gemv(torch::Tensor a_ref, torch::Tensor b_ref, torch::Tensor sfa_ref, torch::Tensor sfb_ref, 
    ...
    const uint32_t SMEM_WIDTH_SFA = K_BLOCK_SIZE/16;
    ...
    const uint32_t SMEM_WIDTH_SFB = 128 * (((K_BLOCK_SIZE/16) + 128 - 1) / 128);
