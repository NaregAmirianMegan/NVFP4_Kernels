...
"""
Support compilation of kernels for varying K_BLOCK_SIZE, RESULTS_PER_WARP, NUM_WARPS_PER_CTA
"""
...
#define K_BLOCK_SIZE 64 // 32 threads per warp * 2 NVFP4 elems per thread = 2 elems per warp (warp handles one k_block)
...
__global__ void batched_nvfp4_gemv_kernel(int m, int k, int l, const __grid_constant__ CUtensorMap tensor_map_a,
    ...
    constexpr int RESULTS_PER_CTA = RESULTS_PER_WARP * NUM_WARPS_PER_CTA;
    ...
    // We need NUM_WARPS_PER_CTA*RESULTS_PER_WARP*(K_BLOCK_SIZE/2) to be a multiple of 128
    // In other words K_BLOCK_SIZE/2 must be a multiple of 128/(NUM_WARPS_PER_CTA*RESULTS_PER_WARP)
    // Assuming NUM_WARPS_PER_CTA*RESULTS_PER_WARP <= 128

    //constexpr int PAD_MULT_A = 128 / RESULTS_PER_CTA;
    constexpr int PAD_MULT_A = 16;

    __shared__ alignas(128) uint8_t a_smem[2][RESULTS_PER_CTA][ PAD_MULT_A * CEIL_DIV(K_BLOCK_SIZE/2, PAD_MULT_A) ];
    __shared__ alignas(128) uint8_t sfa_smem[2][RESULTS_PER_CTA][ PAD_MULT_A * CEIL_DIV(K_BLOCK_SIZE/16, PAD_MULT_A) ];

    __shared__ alignas(128) uint8_t b_smem[2][ 128 * CEIL_DIV(K_BLOCK_SIZE/2, 128) ];
    __shared__ alignas(128) uint8_t sfb_smem[2][ 128 * CEIL_DIV(K_BLOCK_SIZE/16, 128) ];

    __shared__ alignas(128) uint16_t c_smem[RESULTS_PER_CTA];

    //constexpr int total_smem_size = sizeof(uint8_t)*RESULTS_PER_CTA*(K_BLOCK_SIZE/2) + sizeof(uint8_t)*RESULTS_PER_CTA*(K_BLOCK_SIZE/16) + sizeof(uint8_t)*(K_BLOCK_SIZE/2) + sizeof(uint8_t)*(K_BLOCK_SIZE/16);
    constexpr int total_smem_size = sizeof(a_smem[0]) + sizeof(b_smem[0]) + sizeof(sfa_smem[0]) + sizeof(sfb_smem[0]);
    ...
    const int cta_row = blockIdx.x * RESULTS_PER_CTA;
    ...
    for (int kb = 0; kb < k_blocks; kb += 4) {
        ...
        if (kb+4 < k_blocks) {
            ...
            if (threadIdx.x == 0) {
                // Load a block
                cde::cp_async_bulk_tensor_2d_global_to_shared(&a_smem[load_stage], &tensor_map_a, (kb+1)*(K_BLOCK_SIZE/2), cta_row, bar[load_stage]);
                // Load sfa block
                cde::cp_async_bulk_tensor_2d_global_to_shared(&sfa_smem[load_stage], &tensor_map_sfa, (kb+1)*(K_BLOCK_SIZE/16), cta_row, bar[load_stage]);
                // Load b block
                cde::cp_async_bulk_tensor_2d_global_to_shared(&b_smem[load_stage], &tensor_map_b, (kb+1)*(K_BLOCK_SIZE/2), batch, bar[load_stage]);
                // Load sfb block
                cde::cp_async_bulk_tensor_2d_global_to_shared(&sfb_smem[load_stage], &tensor_map_sfb, (kb+1)*(K_BLOCK_SIZE/16), batch, bar[load_stage]);
        ...
        asm volatile(
            ...
            "cvt.u8.u32 e4m3_byte, %1;\\n"
            "mov.b16 e4m3_dup, {e4m3_byte, e4m3_byte};\\n"
            "cvt.rn.f16x2.e4m3x2 %0, e4m3_dup;\\n"
            "}\\n"
                : "=r"(sfb_fp16x2)
                : "r"((uint32_t)sfb_smem[comp_stage][l_wtid/4])
                : "memory"
            );
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            asm volatile(
                ...
                "cvt.u8.u32 sfa_byte, %1;\\n"
                "mov.b16 e4m3_dup, {sfa_byte, sfa_byte};\\n"
                "cvt.rn.f16x2.e4m3x2 sfa_f16x2, e4m3_dup;\\n"

                "mov.b32 accum, 0;\\n"

                "cvt.rn.f16x2.e2m1x2 a_cvt, a_byte;\\n"
                "cvt.rn.f16x2.e2m1x2 b_cvt, b_byte;\\n"

                "mul.rn.f16x2 sf_f16x2, sfa_f16x2, %2;\\n"

                "fma.rn.f16x2 accum, a_cvt, b_cvt, accum;\\n"

                "mul.rn.f16x2 accum, accum, sf_f16x2;\\n"

                "mov.b32 {lane0, lane1}, accum;\\n"
                "add.rn.f16 result_f16, lane0, lane1;\\n"
                "mov.b16 %0, result_f16;\\n"

                "}\\n"
                : "=h"(result_fp16)                                     // 0
                : "r"((uint32_t)sfa_smem[comp_stage][warp_result_offset + result][l_wtid/4]), "r"(sfb_fp16x2),        // 1, 2
                  "r"((uint32_t)a_smem[comp_stage][warp_result_offset + result][l_wtid]),
                  "r"((uint32_t)b_smem[comp_stage][l_wtid])
                : "memory"
            );
    ...
    if (l_wtid == 0) {
        ...
        //c_ref[batch*m + cta_row_start + RESULTS_PER_WARP*l_wid] = results_half[0];
...
torch::Tensor batched_nvfp4_gemv(torch::Tensor a_ref, torch::Tensor b_ref, torch::Tensor sfa_ref, torch::Tensor sfb_ref, 
    ...
    const uint64_t GMEM_WIDTH_A = k/2;
    const uint64_t GMEM_HEIGHT_A = m*l;
    const uint32_t SMEM_WIDTH_A = K_BLOCK_SIZE/2;
    ...
    uint64_t stride_a[rank_a - 1] = {GMEM_WIDTH_A * sizeof(uint8_t)};
    ...
    const uint64_t GMEM_WIDTH_SFA = k/16;
    const uint64_t GMEM_HEIGHT_SFA = m*l;
    const uint32_t SMEM_WIDTH_SFA = 16;
    ...
    // The stride is the number of bytes to traverse from the first element of one row to the next.
    // It must be a multiple of 16.
    uint64_t stride_sfa[rank_sfa - 1] = {GMEM_WIDTH_SFA * sizeof(uint8_t)};
    // The box_size is the size of the shared memory buffer that is used as the
    // destination of a TMA transfer.
    ...
    const uint64_t GMEM_WIDTH_B = k/2;
    const uint64_t GMEM_HEIGHT_B = l;
    const uint32_t SMEM_WIDTH_B = K_BLOCK_SIZE/2;
    ...
    // The stride is the number of bytes to traverse from the first element of one row to the next.
    // It must be a multiple of 16.
    uint64_t stride_b[rank_b - 1] = {GMEM_WIDTH_B * sizeof(uint8_t) * N_DIM_PADDING};
    // The box_size is the size of the shared memory buffer that is used as the
    // destination of a TMA transfer.
    ...
    const uint64_t GMEM_WIDTH_SFB = k/16;
    const uint64_t GMEM_HEIGHT_SFB = l;
    const uint32_t SMEM_WIDTH_SFB = K_BLOCK_SIZE/16;
    ...
    // The stride is the number of bytes to traverse from the first element of one row to the next.
    // It must be a multiple of 16.
    uint64_t stride_sfb[rank_sfb - 1] = {GMEM_WIDTH_SFB * sizeof(uint8_t) * N_DIM_PADDING};
    // The box_size is the size of the shared memory buffer that is used as the
    // destination of a TMA transfer.
