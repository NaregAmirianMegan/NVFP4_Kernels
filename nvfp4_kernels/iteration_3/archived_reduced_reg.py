...
Move vector loads to be explicit in assembly to reduce the amount of data passed
into the volatile assembly block. This didn't seem to decrease the register count.
"""
...
__global__ void batched_nvfp4_gemv_kernel(__half* __restrict__ c_ref, int m, int k, int l, const __grid_constant__ CUtensorMap tensor_map_a,
    ...
    for (int kb = 0; kb < k_blocks; kb++) {
        ...
        uint32_t* a_smem_tmp = reinterpret_cast<uint32_t*>(a_smem);
        uint32_t* b_smem_tmp = reinterpret_cast<uint32_t*>(b_smem);

        a_smem_tmp += warp_result_offset*(K_BLOCK_SIZE/8) + l_wtid*4;
        b_smem_tmp += l_wtid*4;
        ...
        for (int result = 0; result < RESULTS_PER_WARP; result++) {
            ...
            asm volatile(
                ...
                "ld.shared.v4.u32 {a0, a1, a2, a3}, [%3];\\n"
                "ld.shared.v4.u32 {b0, b1, b2, b3}, [%4];\\n"
                ...
                "mov.b32 {a_byte0, a_byte1, a_byte2, a_byte3}, a0;\\n"
                "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, b0;\\n"
                ...
                "mov.b32 {a_byte0, a_byte1, a_byte2, a_byte3}, a1;\\n"
                "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, b1;\\n"
                ...
                "mov.b32 {a_byte0, a_byte1, a_byte2, a_byte3}, a2;\\n"
                "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, b2;\\n"
                ...
                "mov.b32 {a_byte0, a_byte1, a_byte2, a_byte3}, a3;\\n"
                "mov.b32 {b_byte0, b_byte1, b_byte2, b_byte3}, b3;\\n"
                ...
                : "h"(sfa_smem[warp_result_offset + result][l_wtid]), "r"(sfb_fp16x2),        // 1, 2
                  "l"(a_smem_tmp), "l"(b_smem_tmp) // 3, 4
                : "memory"
            );

            a_smem_tmp += K_BLOCK_SIZE/8;
